...
class yogastudio_widget_recent_news extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'yogastudio_widget_recent_news', esc_html__('YogaStudio - Recent News', 'trx_utils'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		if (!empty($output)) {
			...
			if ($widget_title) yogastudio_show_layout($title, $before_title, $after_title);
...
if ( !function_exists( 'yogastudio_sc_recent_news' ) ) {
	function yogastudio_sc_recent_news($atts, $content=null){	
		...
		if (yogastudio_param_is_on($show_categories)) {
			if ( !yogastudio_storage_isset('categories_'.$category) ) {
				yogastudio_storage_set('categories_'.$category, get_categories(array(
					'orderby' => 'name',
					'parent' => $category
					))
				);
		...
		if (empty($from_widget)) {
			$widget_args = yogastudio_prepare_widgets_args(yogastudio_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_news', 'widget_recent_news');
		...
		// Header
		if ( !empty($title) || !empty($subtitle) || (yogastudio_param_is_on($show_categories) && !yogastudio_storage_empty('categories_'.$category)) ) {
			$output	.= '<div class="sc_recent_news_header'.(yogastudio_param_is_on($show_categories) && !yogastudio_storage_empty('categories_'.$category) ? ' sc_recent_news_header_split' : '').'">'
			...
			// Categories list
			$cat_list = yogastudio_storage_get('categories_'.$category);
			if (yogastudio_param_is_on($show_categories) && !empty($cat_list)) {
