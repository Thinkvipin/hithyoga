...
class yogastudio_widget_advert extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'yogastudio_widget_advert', esc_html__('YogaStudio - Advertisement block', 'trx_utils'), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		?>
		<div class="column_wrap banner-output">
			<div class="column-1_2">
				<?php
				$title_exploaded = explode(' ', trim($title));
				yogastudio_show_layout($before_title.			'<span>' . ($title_exploaded[0]) . ' ' .  ($title_exploaded[1]) . ' ' .  ($title_exploaded[2]) . '</span>');
				for( $i = 3; $i <= count($title_exploaded)-1; $i++){
				echo ' ' . ($title_exploaded[$i]);
				}
				echo($after_title);
				echo (!empty($advert_link) ? '<a href="' . esc_url($advert_link) . '" class="sc_services_item_readmore">' : '') .
				(!empty($advert_link) ?  esc_html__('More info','trx_utils'). '<span class="icon-right"></span>' : '')
				. (!empty($advert_link) ?  '</a>': '');
				?>
			...
			<div class="widget_advert_inner">
				...
				if ($advert_image!='') {
					...
					echo '<span class="image_wrap"><img src="' . esc_url($advert_image) . '" alt="' . esc_attr($title) . '"'.(!empty($attr[3]) ? ' '.trim($attr[3]) : '').'></span>';
				}
				if ($advert_code!='') {
...
function form( $instance ) {
	...
	?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_image' )); ?>"><?php echo wp_kses_data( __('Image source URL:<br />(leave empty if you paste advert code)', 'trx_utils') ); ?></label>
            ...
            <?php
			yogastudio_show_layout(yogastudio_show_custom_field($this->get_field_id( 'advert_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'advert_image' )), null));
			if ($advert_image) {
			?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_link' )); ?>"><?php echo wp_kses_data( __('Image link URL:<br />(leave empty if you paste advert code)', 'trx_utils') ); ?></label>
	...
	function yogastudio_sc_widget_advert($atts, $content=null){	
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			...
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, yogastudio_prepare_widgets_args(yogastudio_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_advert', 'widget_advert') );
...
if (!function_exists('yogastudio_widget_advert_reg_shortcodes_vc')) {
	...
	function yogastudio_widget_advert_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Advertisement", 'trx_utils'),
				"description" => wp_kses_data( __("Insert widget with advertisement banner or any HTML and/or Javascript code", 'trx_utils') ),
				...
				"params" => array(
					...
					array(
						...
						"description" => wp_kses_data( __("Select or upload image or write URL from other site for the banner (leave empty if you paste advert code)", 'trx_utils')),
						"class" => "",
						"value" => "",
						"type" => "attach_image"
					),
					array(
						...
						"description" => wp_kses_data( __("Link URL for the banner (leave empty if you paste advert code)", 'trx_utils')),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						"param_name" => "code",
						"heading" => esc_html__("or paste Advert Widget HTML Code", 'trx_utils'),
