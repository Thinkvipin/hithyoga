...
// Theme init
if (!function_exists('yogastudio_widget_categories_theme_setup')) {
	add_action( 'yogastudio_action_before_init_theme', 'yogastudio_widget_categories_theme_setup', 1 );
	function yogastudio_widget_categories_theme_setup() {

		// Add shortcodes in the shortcodes list
		//add_action('yogastudio_action_shortcodes_list',		'yogastudio_widget_categories_reg_shortcodes');
		if (function_exists('yogastudio_exists_visual_composer') && yogastudio_exists_visual_composer())
			add_action('yogastudio_action_shortcodes_list_vc','yogastudio_widget_categories_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('yogastudio_widget_categories_load')) {
	add_action( 'widgets_init', 'yogastudio_widget_categories_load' );
	function yogastudio_widget_categories_load() {
		register_widget( 'yogastudio_widget_categories' );
	}
}

// Widget Class
class yogastudio_widget_categories extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'widget_subcategories', 'description' => esc_html__('Display subcategories list', 'trx_utils') );
		parent::__construct( 'yogastudio_widget_subcategories', esc_html__('YogaStudio - Subcategories list', 'trx_utils'), $widget_ops );
	}
...
*/
if ( !function_exists( 'yogastudio_sc_widget_categories' ) ) {
	function yogastudio_sc_widget_categories($atts, $content=null){	
		...
		extract($atts);
		$type = 'yogastudio_widget_categories';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_categories' 
								. (yogastudio_exists_visual_composer() ? ' vc_widget_categories wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, yogastudio_prepare_widgets_args(yogastudio_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_categories', 'widget_categories') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('yogastudio_shortcode_output', $output, 'trx_widget_categories', $atts, $content);
	}
	yogastudio_require_shortcode("trx_widget_categories", "yogastudio_sc_widget_categories");
}


// Add [trx_widget_categories] in the VC shortcodes list
if (!function_exists('yogastudio_widget_categories_reg_shortcodes_vc')) {
	//add_action('yogastudio_action_shortcodes_list_vc','yogastudio_widget_categories_reg_shortcodes_vc');
	function yogastudio_widget_categories_reg_shortcodes_vc() {
