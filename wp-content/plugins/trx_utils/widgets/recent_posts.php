...
class yogastudio_widget_recent_posts extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'yogastudio_widget_recent_posts', esc_html__('YogaStudio - Recent Posts', 'trx_utils'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		$post_rating = yogastudio_storage_get('options_prefix').'_reviews_avg'.(yogastudio_get_theme_option('reviews_first')=='author' ? '' : '2');
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'trx_utils')), yogastudio_get_list_blog_counters(false));
		?>
	...
	function yogastudio_sc_widget_recent_posts($atts, $content=null){	
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, yogastudio_prepare_widgets_args(yogastudio_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_posts', 'widget_recent_posts') );
