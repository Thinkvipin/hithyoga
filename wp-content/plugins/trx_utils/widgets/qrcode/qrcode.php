 ...
 */
add_action( 'widgets_init', 'yogastudio_widget_qrcode_load' );

/**
 * Register our widget.
 */
function yogastudio_widget_qrcode_load() {
	register_widget('yogastudio_widget_qrcode');
}

/**
 * QRCode Widget class.
 */
class yogastudio_widget_qrcode extends WP_Widget {
	...
	function __construct() {
		...
		$control_ops = array('width' => 200, 'height' => 250, 'id_base' => 'yogastudio_widget_qrcode');

		/* Create the widget. */
		parent::__construct( 'yogastudio_widget_qrcode', esc_html__('YogaStudio - QRCode generator', 'trx_utils'), $widget_ops, $control_ops );
	...
	function widget($args, $instance) {
		...
		$output = '';
			...
			if ($title) yogastudio_show_layout($title, $before_title, $after_title);
		...
		yogastudio_show_layout($before_widget);
	
		yogastudio_show_layout($output);
			
		/* After widget (defined by themes). */
		yogastudio_show_layout($after_widget);
	}
	...
	function form($instance) {
		...
		//wp_enqueue_style('color-picker',        yogastudio_get_file_url('js/colorpicker/colorpicker.css'), array(), null);
		//wp_enqueue_script('color-picker',       yogastudio_get_file_url('js/colorpicker/colorpicker.js'), array('jquery'),null,true);

		/* Set up some default widget settings. */
		$address = explode(',', yogastudio_get_theme_option('user_address'));
		$defaults = array(
			'title' => '', 
		...
		<div class="widget_qrcode">
        	<div class="qrcode_tabs">
                <ul class="tabs">
                    ...
                    <li><a href="#tab_fields" onmousedown="yogastudio_qrcode_init()"><?php esc_html_e('Personal Data', 'trx_utils'); ?></a></li>
                    <li><a href="#tab_text" onmousedown="yogastudio_qrcode_init()"><?php esc_html_e('Any Text', 'trx_utils'); ?></a></li>
                </ul>
                <div id="tab_settings" class="tab_content tab_settings">
                    <p>
                        ...
                        <input class="fld_title" onfocus="yogastudio_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($instance['title']); ?>" style="width:100%;" />
                    </p>
                    <p>
                        <label><?php esc_html_e('Show as QR Code:', 'trx_utils'); ?></label><br />
                        <input class="fld_show_what" onfocus="yogastudio_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_what')); ?>_1" name="<?php echo esc_attr($this->get_field_name('show_what')); ?>" value="1" type="radio" <?php echo (1==$instance['show_what'] ? 'checked="checked"' : ''); ?> />
                        <label for="<?php echo esc_attr($this->get_field_id('show_what')); ?>_1"> <?php esc_html_e('Personal VCard', 'trx_utils'); ?></label>
                        <input class="fld_show_what" onfocus="yogastudio_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_what')); ?>_0" name="<?php echo esc_attr($this->get_field_name('show_what')); ?>" value="0" type="radio" <?php echo (0==$instance['show_what'] ? 'checked="checked"' : ''); ?> />
                        <label for="<?php echo esc_attr($this->get_field_id('show_what')); ?>_0"> <?php esc_html_e('Any text', 'trx_utils'); ?></label>
                    </p>
                    <p>
                        <input class="fld_show_personal" onfocus="yogastudio_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_personal')); ?>" name="<?php echo esc_attr($this->get_field_name('show_personal')); ?>" value="1" type="checkbox" <?php echo (1==$instance['show_personal'] ? 'checked="checked"' : ''); ?> />
                    ...
                    <p>
                        ...
                        <input onmousedown="yogastudio_qrcode_init()" onfocus="yogastudio_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('width')); ?>" name="<?php echo esc_attr($this->get_field_name('width')); ?>" value="<?php echo esc_attr($instance['width']); ?>" style="width:100%;" class="fld_width" />
                    </p>
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('color')); ?>"><?php esc_html_e('Color:', 'trx_utils'); ?></label>
                        <input onmousedown="yogastudio_qrcode_init()" onfocus="yogastudio_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('color')); ?>" name="<?php echo esc_attr($this->get_field_name('color')); ?>" value="<?php echo esc_attr($instance['color']); ?>" style="width:100%; background-color:<?php echo esc_attr($instance['color']); ?>" class="iColorPicker fld_color" />
                    </p>
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('bg')); ?>"><?php esc_html_e('Bg color:', 'trx_utils'); ?></label>
                        <input onmousedown="yogastudio_qrcode_init()" onfocus="yogastudio_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('bg')); ?>" name="<?php echo esc_attr($this->get_field_name('bg')); ?>" value="<?php echo esc_attr($instance['bg']); ?>" style="width:100%; background-color:<?php echo esc_attr($instance['bg']); ?>" class="iColorPicker fld_bg" />
                    </p>
                </div>
            ...
            <div id="<?php echo esc_attr($this->get_field_id('qrcode_image')); ?>" class="qrcode_image"><img src="<?php yogastudio_show_layout($instance['image']); ?>" alt="" /></div>
...
<?php if ($instance['show_personal']==1) { ?>
				...
				<?php if ($instance['show_what']==1) { ?>
					...
					?>
				<?php } else { ?>
                    <li class="text odd first"><?php yogastudio_show_layout($instance['text']); ?></li>
				<?php } ?>
                </ul>
<?php } ?>
            </div>
        ...
        <script type="text/javascript">
            jQuery(document).ready(function(){
				yogastudio_qrcode_init();
				yogastudio_color_picker();
            });
			function yogastudio_qrcode_init() {
            ...
            function yogastudio_qrcode_update(widget) {
				yogastudio_qrcode_show(widget, {
			...
			function yogastudio_qrcode_show(widget, vc, opt) {
				if (opt.show_what==1) {
			...
			if (!window.yogastudio_set_color_picker) {
				function yogastudio_set_color_picker(id_picker) {
