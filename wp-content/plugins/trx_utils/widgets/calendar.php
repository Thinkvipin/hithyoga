...
class yogastudio_widget_calendar extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'yogastudio_widget_calendar', esc_html__('YogaStudio - Advanced Calendar', 'trx_utils'), $widget_ops );
	}
	...
	function yogastudio_sc_widget_calendar($atts, $content=null){	
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, yogastudio_prepare_widgets_args(yogastudio_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_calendar', 'widget_calendar') );
