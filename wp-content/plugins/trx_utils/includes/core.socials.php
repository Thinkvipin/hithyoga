<?php
/**
 * Yogastudio Framework: social networks
 *
 * @package	yogastudio
...
// Theme init
if (!function_exists('yogastudio_socials_theme_setup')) {
	add_action( 'yogastudio_action_before_init_theme', 'yogastudio_socials_theme_setup' );
	function yogastudio_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 					'yogastudio_facebook_og_tags', 5 );
		}
...
-------------------------------------------------------------------------------- */
if (!function_exists('yogastudio_socials_theme_setup1')) {
    add_action( 'yogastudio_action_before_init_theme', 'yogastudio_socials_theme_setup1' );
    function yogastudio_socials_theme_setup1() {
        // List of social networks for site sharing and user profiles
        yogastudio_storage_set('share_links', array(
            ...
            'postila' =>		'http://postila.ru/publish/?url={link}&agregator=yogastudio',
...
if (!function_exists('yogastudio_add_share_link')) {
	function yogastudio_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		yogastudio_storage_set('share_links', array_merge( yogastudio_storage_get('share_links'), $soc ) );
	}
}

// Return (and show) share social links
if (!function_exists('yogastudio_show_share_links')) {
	function yogastudio_show_share_links($args) {
		if ( yogastudio_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'counters' => yogastudio_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => yogastudio_get_custom_option('show_share'),				// share block direction
			'caption' => yogastudio_get_custom_option('share_caption'),				// share block caption
			'share' => yogastudio_get_theme_option('share_buttons'),					// list of allowed socials
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			foreach ($args['share'] as $soc) {
				...
				$url = $soc['url'];
				if (empty($url)) $url = yogastudio_storage_get_array('share_links', $sn);
				...
				$output .= '<div class="sc_socials_item' . (!empty($args['popup']) ? ' social_item_popup' : '') . '">'
					...
					. ($args['type']=='drop' ? '<i>' . trim(yogastudio_strtoproper($sn)) . '</i>' : '')
					. '</div>';
			}
		}
		$output .= '</div>';
		if ($args['echo']) yogastudio_show_layout($output);
		return $output;
	}
}
...
if (!function_exists('yogastudio_prepare_socials')) {
	function yogastudio_prepare_socials($list, $style='') {
		if (empty($style)) $style = yogastudio_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || yogastudio_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : yogastudio_get_socials_url(basename($soc['icon']));
				if ($style == 'icons') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $parts[0];
				} else {
...
if (!function_exists('yogastudio_get_twitter_data')) {
	function yogastudio_get_twitter_data($cfg) {
...
if (!function_exists('yogastudio_prepare_twitter_text')) {
	function yogastudio_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('yogastudio_get_twitter_followers')) {
	function yogastudio_get_twitter_followers($cfg) {
		$data = yogastudio_get_twitter_data($cfg); 
...
if (!function_exists('yogastudio_get_facebook_likes')) {
	function yogastudio_get_facebook_likes($account) {
		$fb = get_transient("facebooklikes");
		if ($fb !== false) return $fb;
		$fb = '?';
		$url = esc_url(yogastudio_get_protocol().'http://graph.facebook.com/'.($account));
		$headers = get_headers($url);
		if (yogastudio_strpos($headers[0], '200')) {
			$json = yogastudio_fgc($url);
...
if (!function_exists('yogastudio_facebook_og_tags')) {
	function yogastudio_facebook_og_tags() {
		global $post;
		if ( !is_singular() || yogastudio_storage_get('blog_streampage')) return;
...
if (!function_exists('yogastudio_get_feedburner_counter')) {
	function yogastudio_get_feedburner_counter($account) {
		$rss = get_transient("feedburnercounter");
		if ($rss !== false) return $rss;
		$rss = '?';
		$url = esc_url(yogastudio_get_protocol().'://feedburner.google.com/api/awareness/1.0/GetFeedData?uri='.($account));
		$headers = get_headers($url);
		if (yogastudio_strpos($headers[0], '200')) {
			$xml = yogastudio_fgc($url);
