...
jQuery(document).ready(function(){
	...
	YOGASTUDIO_STORAGE['shortcodes_delimiter'] = ',';		// Delimiter for multiple values
	YOGASTUDIO_STORAGE['shortcodes_popup'] = null;		// Popup with current shortcode settings
	YOGASTUDIO_STORAGE['shortcodes_current_idx'] = '';	// Current shortcode's index
	YOGASTUDIO_STORAGE['shortcodes_tab_clone_tab'] = '<li id="yogastudio_shortcodes_tab_{id}" data-id="{id}"><a href="#yogastudio_shortcodes_tab_{id}_content"><span class="iconadmin-{icon}"></span>{title}</a></li>';
	YOGASTUDIO_STORAGE['shortcodes_tab_clone_content'] = '';

	// Shortcode selector - "change" event handler - add selected shortcode in editor
	jQuery('body').on('change', ".sc_selector", function() {
		"use strict";
		YOGASTUDIO_STORAGE['shortcodes_current_idx'] = jQuery(this).find(":selected").val();
		if (YOGASTUDIO_STORAGE['shortcodes_current_idx'] == '') return;
		var sc = yogastudio_clone_object(YOGASTUDIO_STORAGE['shortcodes'][YOGASTUDIO_STORAGE['shortcodes_current_idx']]);
		var hdr = sc.title;
		var content = "";
		try {
		...
		// Show Dialog popup
		YOGASTUDIO_STORAGE['shortcodes_popup'] = yogastudio_message_dialog(html, hdr,
			function(popup) {
				"use strict";
			...
			function(btn, popup) {
				"use strict";
				if (btn != 1) return;
				var sc = yogastudio_shortcodes_get_code(YOGASTUDIO_STORAGE['shortcodes_popup']);
		...
		// Add new child tab
		YOGASTUDIO_STORAGE['shortcodes_popup'].find('.yogastudio_shortcodes_tab').on('tabsbeforeactivate', function (e, ui) {
		...
		// Delete child tab
		YOGASTUDIO_STORAGE['shortcodes_popup'].find('.yogastudio_shortcodes_tab > ul').on('click', '> li+li > a > span', function (e) {
...
function yogastudio_shortcodes_get_code(popup) {
	...
	if (yogastudio_isset(sc.children)) {
		if (YOGASTUDIO_STORAGE['sc_custom']!='no') {
	...
	if (yogastudio_isset(sc.container) && sc.container || yogastudio_isset(sc.children))
		rez += 
			(yogastudio_isset(sc.children) && decor && YOGASTUDIO_STORAGE['sc_custom']!='no' ? '\n' : '')
			+ '[/' + sc_name + ']'
			 //+ (decor ? '\n' : '')
			 ;
	return rez;
...
function yogastudio_shortcodes_get_code_from_tab(tab) {
	...
	tab.find('[data-param]').each(function () {
		...
		if (mainTab && param=='custom') {
			YOGASTUDIO_STORAGE['sc_custom'] = field.val();
		}
	});
	// Get additional params for general tab from items tabs
	if (YOGASTUDIO_STORAGE['sc_custom']!='no' && mainTab) {
		var sc = YOGASTUDIO_STORAGE['shortcodes'][YOGASTUDIO_STORAGE['shortcodes_current_idx']];
		var sc_name = YOGASTUDIO_STORAGE['shortcodes_current_idx'];
...
function yogastudio_shortcodes_prepare_layout(field) {
	...
	if (!yogastudio_empty(field.children)) {
		YOGASTUDIO_STORAGE['shortcodes_tab_clone_content'] = yogastudio_shortcodes_show_field(field.children, 1);
		output += YOGASTUDIO_STORAGE['shortcodes_tab_clone_content'];
	}
	output += '</div></form></div>';
	return output;
}
...
function yogastudio_shortcodes_show_tabs(field) {
	...
	var output = '<div class="yogastudio_shortcodes_tab yogastudio_options_container yogastudio_options_tab">'
		+ '<ul>'
		+ YOGASTUDIO_STORAGE['shortcodes_tab_clone_tab'].replace(/{id}/g, 0).replace('{icon}', 'cog').replace('{title}', 'General');
	if (yogastudio_isset(field.children)) {
		for (var i=0; i<field.children.params.length; i++)
			output += YOGASTUDIO_STORAGE['shortcodes_tab_clone_tab'].replace(/{id}/g, i+1).replace('{icon}', 'cancel').replace('{title}', field.children.title + ' ' + (i+1));
		output += YOGASTUDIO_STORAGE['shortcodes_tab_clone_tab'].replace(/{id}/g, 'add').replace('{icon}', 'list-add').replace('{title}', '');
	}
	output += '</ul>';
	return output;
}
...
function yogastudio_shortcodes_add_tab(tab) {
	...
	idx++;
	tab.before( YOGASTUDIO_STORAGE['shortcodes_tab_clone_tab'].replace(/{id}/g, idx).replace('{icon}', 'cancel').replace('{title}', YOGASTUDIO_STORAGE['shortcodes'][YOGASTUDIO_STORAGE['shortcodes_current_idx']].children.title + ' ' + idx) );
	tab.parents('.yogastudio_shortcodes_tab').append(YOGASTUDIO_STORAGE['shortcodes_tab_clone_content'].replace(/tab_1_/g, 'tab_' + idx + '_'));
...
function yogastudio_shortcodes_show_field(field, tab_idx) {
	...
	for (var clone_num in field['params']) {
		...
		for (var param_num in field['params'][clone_num]) {
			...
			if (param['type']=='color' && (YOGASTUDIO_STORAGE['shortcodes_cp']=='tiny' || (yogastudio_isset(param['style']) && param['style']!='wp'))) {
				if (!yogastudio_isset(param['after'])) param['after'] = {};
				param['after'] = yogastudio_merge_objects({
						'icon': 'iconadmin-cancel',
						'action': 'color_reset'
			...
			case 'tags':
				var tags = param['value'].split(YOGASTUDIO_STORAGE['shortcodes_delimiter']);
				if (tags.length > 0) {
			...
			case 'range':
				...
				if (param['value'].toString().indexOf(YOGASTUDIO_STORAGE['shortcodes_delimiter']) == -1)
					param['value'] = Math.min(param['max'], Math.max(param['min'], param['value']));
				var sliders = param['value'].toString().split(YOGASTUDIO_STORAGE['shortcodes_delimiter']);
			...
			case "checklist":
				for (key in param['options']) { 
					output += '<span class="yogastudio_options_listitem'
						+ (yogastudio_in_list(param['value'], key, YOGASTUDIO_STORAGE['shortcodes_delimiter']) ? ' yogastudio_options_state_checked' : '') + '"'
						+ ' data-value="' + yogastudio_shortcodes_prepare_value(key) + '"'
						+ '>'
						+ param['options'][key]
						+ '</span>';
			...
			case "color":
				var cp_style = yogastudio_isset(param['style']) ? param['style'] : YOGASTUDIO_STORAGE['shortcodes_cp'];
...
function yogastudio_shortcodes_menu_list(field) {
	...
	for (var key in field['options']) {
		...
		var selected = '';
		if (yogastudio_in_list(field['value'], key, YOGASTUDIO_STORAGE['shortcodes_delimiter'])) {
			caption = value;
