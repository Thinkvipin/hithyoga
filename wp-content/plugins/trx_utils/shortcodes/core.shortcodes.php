...
if ( !function_exists( 'yogastudio_sc_add_styles' ) ) {
	...
	function yogastudio_sc_add_styles() {
		// Shortcodes
		wp_enqueue_style( 'yogastudio-shortcodes-style',	trx_utils_get_file_url('shortcodes/theme.shortcodes.css'), array(), null );
	}
}
...
if ( !function_exists( 'yogastudio_sc_add_scripts' ) ) {
	...
	function yogastudio_sc_add_scripts($output, $tag='', $atts=array(), $content='') {
		...
		if (yogastudio_storage_empty('shortcodes_scripts_added')) {
			yogastudio_storage_set('shortcodes_scripts_added', true);
			wp_enqueue_script( 'yogastudio-shortcodes-script', trx_utils_get_file_url('shortcodes/theme.shortcodes.js'), array('jquery'), null, true );
		}
...
if (!function_exists('yogastudio_sc_load_scripts')) {
	function yogastudio_sc_load_scripts() {
		wp_enqueue_script( 'yogastudio-shortcodes-script', trx_utils_get_file_url('shortcodes/shortcodes_admin.js'), array('jquery'), null, true );
		wp_enqueue_script( 'yogastudio-selection-script',  yogastudio_get_file_url('js/jquery.selection.js'), array('jquery'), null, true );
	}
}
...
if (!function_exists('yogastudio_sc_prepare_scripts')) {
	function yogastudio_sc_prepare_scripts() {
		if (!yogastudio_storage_isset('shortcodes_prepared')) {
			yogastudio_storage_set('shortcodes_prepared', true);
			$json_parse_func = 'eval';	// 'JSON.parse'
			?>
			<script type="text/javascript">
				jQuery(document).ready(function(){
					try {
						YOGASTUDIO_STORAGE['shortcodes'] = <?php yogastudio_show_layout($json_parse_func); ?>(<?php echo json_encode( yogastudio_array_prepare_to_json(yogastudio_storage_get('shortcodes')) ); ?>);
					} catch (e) {}
					YOGASTUDIO_STORAGE['shortcodes_cp'] = '<?php echo is_admin() ? (!yogastudio_storage_empty('to_colorpicker') ? yogastudio_storage_get('to_colorpicker') : 'wp') : 'custom'; ?>';	// wp | tiny | custom
				});
			</script>
			<?php
		}
...
if (!function_exists('yogastudio_sc_selector_add_in_toolbar')) {
	...
	function yogastudio_sc_selector_add_in_toolbar(){
		...
		$shortcodes = yogastudio_storage_get('shortcodes');
		...
		yogastudio_show_layout($shortcodes_list);
	}
}
...
require_once trx_utils_get_file_dir('shortcodes/trx_basic/scheduleitem.php');
