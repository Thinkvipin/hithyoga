...
if (!function_exists('yogastudio_sc_accordion')) {	
	function yogastudio_sc_accordion($atts, $content=null){	
		...
		yogastudio_storage_set('sc_accordion_data', array(
			'counter' => 0,
            'show_counter' => yogastudio_param_is_on($counter),
            'icon_closed' => empty($icon_closed) || yogastudio_param_is_inherit($icon_closed) ? "icon-plus" : $icon_closed,
            'icon_opened' => empty($icon_opened) || yogastudio_param_is_inherit($icon_opened) ? "icon-minus" : $icon_opened
            )
        );
...
if (!function_exists('yogastudio_sc_accordion_item')) {	
	function yogastudio_sc_accordion_item($atts, $content=null) {
		...
		yogastudio_storage_inc_array('sc_accordion_data', 'counter');
		if (empty($icon_closed) || yogastudio_param_is_inherit($icon_closed)) $icon_closed = yogastudio_storage_get_array('sc_accordion_data', 'icon_closed', '', "icon-down-dir");
		if (empty($icon_opened) || yogastudio_param_is_inherit($icon_opened)) $icon_opened = yogastudio_storage_get_array('sc_accordion_data', 'icon_opened', '', "icon-up-dir");
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
				. ' class="sc_accordion_item'
				. (!empty($class) ? ' '.esc_attr($class) : '')
				. (yogastudio_storage_get_array('sc_accordion_data', 'counter') % 2 == 1 ? ' odd' : ' even')
				. (yogastudio_storage_get_array('sc_accordion_data', 'counter') == 1 ? ' first' : '')
				. '">'
				...
				. (yogastudio_storage_get_array('sc_accordion_data', 'show_counter') ? '<span class="sc_items_counter">'.(yogastudio_storage_get_array('sc_accordion_data', 'counter')).'</span>' : '')
				. ($title)
				. '</h4>'
...
if ( !function_exists( 'yogastudio_sc_accordion_reg_shortcodes' ) ) {
	...
	function yogastudio_sc_accordion_reg_shortcodes() {
		...
		yogastudio_sc_map("trx_accordion", array(
