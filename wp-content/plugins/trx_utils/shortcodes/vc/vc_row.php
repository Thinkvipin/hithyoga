...
if (function_exists('vc_map_get_attributes')) {						// New VC
	...
	// Add Yogastudio color scheme in the row
	if ($scheme && !yogastudio_param_is_off($scheme) && !yogastudio_param_is_inherit($scheme)) $css_classes[] = 'scheme_'.esc_attr($scheme);
	...
	// Add Yogastudio color scheme in the row
	if ($scheme && !yogastudio_param_is_off($scheme) && !yogastudio_param_is_inherit($scheme)) $css_class .= ' scheme_'.esc_attr($scheme);
	...
	} ?> <?php yogastudio_show_layout($style); ?>><?php
	echo wpb_js_remove_wpautop( $content );
	?></div><?php yogastudio_show_layout($this->endBlockComment('row'));
	if ( ! empty( $full_width ) ) {
