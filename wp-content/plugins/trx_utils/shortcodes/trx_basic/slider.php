	...
	function yogastudio_sc_slider($atts, $content=null){	
		...
		$output = (in_array($pagination, array('full', 'over')) 
				...
				. ' class="sc_slider sc_slider_' . esc_attr($engine)
					...
					. (yogastudio_storage_get_array('sc_slider_data', 'bg_image') ? ' sc_slider_bg' : ' sc_slider_images')
		...
		if ($engine=='revo') {
			...
			$output .= '<div class="slides'
				...
				.($engine=='swiper' && yogastudio_storage_get_array('sc_slider_data', 'bg_image') ? ' style="'.esc_attr($hs).'"' : '')
				.'>';
			...
			if (yogastudio_param_is_on($custom) && $content) {
				...
				while ( $query->have_posts() ) { 
					...
					if (yogastudio_param_is_on($crop)) {
						$post_attachment = yogastudio_storage_get_array('sc_slider_data', 'bg_image')
					...
					} else if (!yogastudio_storage_get_array('sc_slider_data', 'bg_image')) {
					...
					$output .= '<div' 
						...
						. ' style="'
							. (yogastudio_storage_get_array('sc_slider_data', 'bg_image') ? 'background-image:url(' . esc_url($post_attachment) . ');' : '') . ($ws) . ($hs)
							. '"'
						. '>' 
						. (yogastudio_param_is_on($links) ? '<a href="'.esc_url($post_link).'" title="'.esc_attr($post_title).'">' : '')
						. (!yogastudio_storage_get_array('sc_slider_data', 'bg_image') ? $post_attachment : '')
						;
...
if (!function_exists('yogastudio_sc_slider_item')) {	
	function yogastudio_sc_slider_item($atts, $content=null) {
		...
		$engine = yogastudio_storage_get_array('sc_slider_data', 'engine');
		$crop_image = yogastudio_storage_get_array('sc_slider_data', 'crop_image');
		$bg_image = yogastudio_storage_get_array('sc_slider_data', 'bg_image');
		$links = yogastudio_storage_get_array('sc_slider_data', 'links');
		$width = yogastudio_storage_get_array('sc_slider_data', 'width');
		$height = yogastudio_storage_get_array('sc_slider_data', 'height');

		if ($src && yogastudio_param_is_on($crop_image)) {
			$src = yogastudio_storage_get_array('sc_slider_data', 'bg_image')
				? yogastudio_get_resized_image_url($src, !empty($width) && yogastudio_strpos($width, '%')===false ? $width : null, !empty($height) && yogastudio_strpos($height, '%')===false ? $height : null)
				: yogastudio_get_resized_image_tag($src, !empty($width) && yogastudio_strpos($width, '%')===false ? $width : null, !empty($height) && yogastudio_strpos($height, '%')===false ? $height : null);
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '').' class="'.esc_attr($engine).'-slide' . (!empty($class) ? ' '.esc_attr($class) : '') . '"'
				. ($css ? ' style="'.esc_attr($css).'"' : '')
				.'>' 
				. ($src && yogastudio_param_is_on($links) ? '<a href="'.esc_url($src).'">' : '')
				. ($src && !$bg_image ? $src : $content)
				. ($src && yogastudio_param_is_on($links) ? '</a>' : '')
			. '</div>';
...
if ( !function_exists( 'yogastudio_sc_slider_reg_shortcodes' ) ) {
	...
	function yogastudio_sc_slider_reg_shortcodes() {
		...
		yogastudio_sc_map("trx_slider", array(
			...
			"params" => array_merge(array(
				"engine" => array(
					...
					"value" => "swiper",
					"type" => "checklist",
					"options" => yogastudio_get_sc_param('sliders')
				),
				"align" => array(
				...
				"alias" => array(
					...
					"value" => "",
					"type" => "select",
					"options" => yogastudio_get_sc_param('revo_sliders')
				)) : array(), array(
				"cat" => array(
					...
					"style" => "list",
					"multiple" => true,
					"options" => yogastudio_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), yogastudio_get_sc_param('categories'))
				),
				"count" => array(
		...
		vc_map( array(
			...
			"params" => array_merge(array(
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(yogastudio_get_sc_param('sliders')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "align",
				...
				array(
					...
					"value" => array_flip(yogastudio_array_merge(array('none' => esc_html__('- Select slider -', 'trx_utils')), yogastudio_get_sc_param('revo_sliders'))),
					"type" => "dropdown"
				)) : array(), array(
				array(
					"param_name" => "cat",
					...
					"class" => "",
					"value" => array_flip(yogastudio_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), yogastudio_get_sc_param('categories'))),
					"type" => "dropdown"
				),
				array(
					"param_name" => "count",
