...
if (!function_exists('yogastudio_sc_columns')) {	
	function yogastudio_sc_columns($atts, $content=null){	
		...
		yogastudio_storage_set('sc_columns_data', array(
			'counter' => 1,
            'after_span2' => false,
            'after_span3' => false,
            'after_span4' => false,
...
if (!function_exists('yogastudio_sc_column_item')) {	
	function yogastudio_sc_column_item($atts, $content=null) {
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') . ' class="column-'.($span > 1 ? esc_attr($span) : 1).'_'.esc_attr(yogastudio_storage_get_array('sc_columns_data', 'count')).' sc_column_item sc_column_item_'.esc_attr(yogastudio_storage_get_array('sc_columns_data', 'counter'))
					. (!empty($class) ? ' '.esc_attr($class) : '')
					. (yogastudio_storage_get_array('sc_columns_data', 'counter') % 2 == 1 ? ' odd' : ' even')
					. (yogastudio_storage_get_array('sc_columns_data', 'counter') == 1 ? ' first' : '')
					. ($span > 1 ? ' span_'.esc_attr($span) : '') 
					. (yogastudio_storage_get_array('sc_columns_data', 'after_span2') ? ' after_span_2' : '')
					. (yogastudio_storage_get_array('sc_columns_data', 'after_span3') ? ' after_span_3' : '')
					. (yogastudio_storage_get_array('sc_columns_data', 'after_span4') ? ' after_span_4' : '')
					. '"'
					. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
					. (!yogastudio_param_is_off($animation) ? ' data-animation="'.esc_attr(yogastudio_get_animation_classes($animation)).'"' : '')
					. '>'
					...
					. '</div>';
		yogastudio_storage_inc_array('sc_columns_data', 'counter', $span);
		yogastudio_storage_set_array('sc_columns_data', 'after_span2', $span==2);
		yogastudio_storage_set_array('sc_columns_data', 'after_span3', $span==3);
		yogastudio_storage_set_array('sc_columns_data', 'after_span4', $span==4);
...
if ( !function_exists( 'yogastudio_sc_columns_reg_shortcodes' ) ) {
	...
	function yogastudio_sc_columns_reg_shortcodes() {
		...
		yogastudio_sc_map("trx_columns", array(
