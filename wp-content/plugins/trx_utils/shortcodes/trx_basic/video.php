...
if (!function_exists('yogastudio_sc_video')) {	
	function yogastudio_sc_video($atts, $content = null) {
		...
		if ($image!='' && yogastudio_param_is_off($image))
			...
			if (yogastudio_param_is_on($autoplay) && is_singular() && !yogastudio_storage_get('blog_streampage'))
				$image = '';
			else {
				if ($image > 0) {
				...
				if (empty($image) && (!is_singular() || yogastudio_storage_get('blog_streampage')))	// || yogastudio_param_is_off($autoplay)))
		...
		$video = '<video' . ($id ? ' id="' . esc_attr($id) . '"' : '') 
			...
			. (($image && yogastudio_param_is_on(yogastudio_get_custom_option('substitute_video'))) || (yogastudio_param_is_on($autoplay) && is_singular() && !yogastudio_storage_get('blog_streampage')) ? ' autoplay="autoplay"' : '')
...
if ( !function_exists( 'yogastudio_sc_video_reg_shortcodes' ) ) {
	...
	function yogastudio_sc_video_reg_shortcodes() {
		...
		yogastudio_sc_map("trx_video", array(
