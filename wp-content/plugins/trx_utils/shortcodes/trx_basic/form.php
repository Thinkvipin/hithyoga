...
if (!function_exists('yogastudio_sc_form')) {	
	function yogastudio_sc_form($atts, $content = null) {
		...
		yogastudio_storage_set('sc_form_data', array(
			'id' => $id,
            'counter' => 0
            )
        );
...
if (!function_exists('yogastudio_sc_form_item')) {	
	function yogastudio_sc_form_item($atts, $content=null) {
		...
		yogastudio_storage_inc_array('sc_form_data', 'counter');
	
		$class .= ($class ? ' ' : '') . yogastudio_get_css_position_as_classes($top, $right, $bottom, $left);
		if (empty($id)) $id = yogastudio_storage_get_array('sc_form_data', 'id').'_'.yogastudio_storage_get_array('sc_form_data', 'counter');
		...
		// Label top or left
		if ($type!='button' && $type!='submit' && ($label_position=='top' || $label_position=='left' || $label_position=='over'))
			$output .= $label;
		...
		if ($type == 'textarea')
			...
			wp_enqueue_script( 'jquery-picker', yogastudio_get_file_url('/js/picker/picker.js'), array('jquery'), null, true );
			wp_enqueue_script( 'jquery-picker-date', yogastudio_get_file_url('/js/picker/picker.date.js'), array('jquery'), null, true );
			...
			wp_enqueue_script( 'jquery-picker', yogastudio_get_file_url('/js/picker/picker.js'), array('jquery'), null, true );
			wp_enqueue_script( 'jquery-picker-time', yogastudio_get_file_url('/js/picker/picker.time.js'), array('jquery'), null, true );
...
if ( !function_exists( 'yogastudio_sc_form_send' ) ) {
	function yogastudio_sc_form_send() {
		...
		if (!($contact_email = yogastudio_get_theme_option('contact_email')) && !($contact_email = yogastudio_get_theme_option('admin_email'))) 
			...
			$mail = yogastudio_get_theme_option('mail_function') == 'mail' ? 'mail' : 'wp_mail';
			if (is_email($contact_email) && !@$mail($contact_email, $subj, apply_filters('yogastudio_filter_form_send_message', $msg))) {
...
if ( !function_exists( 'yogastudio_sc_form_reg_shortcodes' ) ) {
	...
	function yogastudio_sc_form_reg_shortcodes() {
		...
		yogastudio_sc_map("trx_form", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"divider" => true,
					"value" => 'form_custom',
					"options" => yogastudio_get_sc_param('forms'),
					"type" => "checklist"
				), 
				"scheme" => array(
			...
			"children" => array(
				...
				"params" => array(
					"type" => array(
						...
						"type" => "checklist",
						"dir" => "horizontal",
						"options" => yogastudio_get_sc_param('field_types')
					), 
					"name" => array(
					...
					"label_position" => array(
						...
						"type" => "checklist",
						"dir" => "horizontal",
						"options" => yogastudio_get_sc_param('label_positions')
					), 
					"top" => yogastudio_get_sc_param('top'),
					"bottom" => yogastudio_get_sc_param('bottom'),
					"left" => yogastudio_get_sc_param('left'),
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"std" => "form_custom",
					"value" => array_flip(yogastudio_get_sc_param('forms')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scheme",
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(yogastudio_get_sc_param('field_types')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "name",
				...
				array(
					...
					"class" => "",
					"value" => array_flip(yogastudio_get_sc_param('label_positions')),
					"type" => "dropdown"
				),
				yogastudio_get_vc_param('id'),
				yogastudio_get_vc_param('class'),
