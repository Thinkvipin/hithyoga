...
if (!function_exists('yogastudio_sc_socials')) {	
	function yogastudio_sc_socials($atts, $content=null){	
		...
		yogastudio_storage_set('sc_social_data', array(
			'icons' => false,
            'type' => $type
            )
        );
		if (!empty($socials)) {
			...
			for ($i=0; $i<count($allowed); $i++) {
				...
				if (!empty($s[1])) {
					$list[] = array(
						'icon'	=> $type=='images' ? yogastudio_get_socials_url($s[0]) : 'icon-'.trim($s[0]),
						'url'	=> $s[1]
						);
				}
			}
			if (count($list) > 0) yogastudio_storage_set_array('sc_social_data', 'icons', $list);
		} else if (yogastudio_param_is_off($custom))
			$content = do_shortcode($content);
		if (yogastudio_storage_get_array('sc_social_data', 'icons')===false) yogastudio_storage_set_array('sc_social_data', 'icons', yogastudio_get_custom_option('social_icons'));
		$output = yogastudio_prepare_socials(yogastudio_storage_get_array('sc_social_data', 'icons'));
		$output = $output
...
if (!function_exists('yogastudio_sc_social_item')) {	
	function yogastudio_sc_social_item($atts, $content=null){	
		...
		if (!empty($name) && empty($icon)) {
			$type = yogastudio_storage_get_array('sc_social_data', 'type');
			if ($type=='images') {
		...
		if (!empty($icon) && !empty($url)) {
			if (yogastudio_storage_get_array('sc_social_data', 'icons')===false) yogastudio_storage_set_array('sc_social_data', 'icons', array());
			yogastudio_storage_set_array2('sc_social_data', 'icons', '', array(
				'icon' => $icon,
				'url' => $url
				)
			);
...
if ( !function_exists( 'yogastudio_sc_socials_reg_shortcodes' ) ) {
	...
	function yogastudio_sc_socials_reg_shortcodes() {
		...
		yogastudio_sc_map("trx_socials", array(
			...
			"params" => array(
				...
				"size" => array(
					...
					"value" => "small",
					"options" => yogastudio_get_sc_param('sizes'),
					"type" => "checklist"
				), 
				"shape" => array(
					...
					"value" => "square",
					"options" => yogastudio_get_sc_param('shapes'),
					"type" => "checklist"
				), 
				"socials" => array(
		...
		vc_map( array(
			...
			"params" => array_merge(array(
				...
				array(
					...
					"class" => "",
					"std" => "small",
					"value" => array_flip(yogastudio_get_sc_param('sizes')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "shape",
					...
					"class" => "",
					"std" => "square",
					"value" => array_flip(yogastudio_get_sc_param('shapes')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "socials",
