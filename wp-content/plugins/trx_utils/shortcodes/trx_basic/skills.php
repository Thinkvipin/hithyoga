...
if (!function_exists('yogastudio_sc_skills')) {	
	function yogastudio_sc_skills($atts, $content=null){	
		...
		yogastudio_storage_set('sc_skills_data', array(
			'counter' => 0,
            'columns' => 0,
            'height'  => 0,
            'type'    => $type,
            'pie_compact' => yogastudio_param_is_on($pie_compact) ? 'on' : 'off',
		...
		if ($type!='arc') {
			...
			if ($layout=='columns') yogastudio_storage_set_array('sc_skills_data', 'columns', $columns);
			if ($type=='bar') {
		...
		if ($style) {
			...
			yogastudio_storage_set_array('sc_skills_data', 'style', $style);
		}
		yogastudio_storage_set_array('sc_skills_data', 'max', $max_value);
		yogastudio_storage_set_array('sc_skills_data', 'dir', $dir);
		yogastudio_storage_set_array('sc_skills_data', 'height', yogastudio_prepare_css_value($height));
		...
		if (!yogastudio_storage_empty('sc_skills_data', 'height') && (yogastudio_storage_get_array('sc_skills_data', 'type') == 'arc' || (yogastudio_storage_get_array('sc_skills_data', 'type') == 'pie' && yogastudio_param_is_on(yogastudio_storage_get_array('sc_skills_data', 'pie_compact')))))
			$css .= 'height: '.yogastudio_storage_get_array('sc_skills_data', 'height');
		$content = do_shortcode($content);
		$output = '<div id="'.esc_attr($id).'"' 
					. ' class="sc_skills sc_skills_' . esc_attr($type) 
						. ($type=='bar' ? ' sc_skills_'.esc_attr($dir) : '') 
						. ($type=='pie' ? ' sc_skills_compact_'.esc_attr(yogastudio_storage_get_array('sc_skills_data', 'pie_compact')) : '')
				...
				. '>'
					...
					. ($type=='arc' 
						? ('<div class="sc_skills_legend">'.(yogastudio_storage_get_array('sc_skills_data', 'legend')).'</div>'
							. '<div id="'.esc_attr($id).'_diagram" class="sc_skills_arc_canvas"></div>'
							. '<div class="sc_skills_data" style="display:none;">' . (yogastudio_storage_get_array('sc_skills_data', 'data')) . '</div>'
						  )
						: '')
					. ($type=='pie' && yogastudio_param_is_on(yogastudio_storage_get_array('sc_skills_data', 'pie_compact'))
						? ('<div class="sc_skills_legend">'.(yogastudio_storage_get_array('sc_skills_data', 'legend')).'</div>'
...
if (!function_exists('yogastudio_sc_skills_item')) {	
	function yogastudio_sc_skills_item($atts, $content=null) {
		...
		yogastudio_storage_set_array('sc_skills_data', 'bg_color', $bg_color);
		yogastudio_storage_set_array('sc_skills_data', 'border_color', $border_color);
		yogastudio_storage_inc_array('sc_skills_data', 'counter');

		$ed = yogastudio_substr($value, -1)=='%' ? '%' : '';
		$value = str_replace('%', '', $value);
		if (yogastudio_storage_get_array('sc_skills_data', 'max') < $value) yogastudio_storage_set_array('sc_skills_data', 'max', $value);
		$percent = round($value / yogastudio_storage_get_array('sc_skills_data', 'max') * 100);
		$start = 0;
		$stop = $value;
		$steps = 100;
		$step = max(1, round(yogastudio_storage_get_array('sc_skills_data', 'max')/$steps));
		$speed = mt_rand(10,40);
		$animation = round(($stop - $start) / $step * $speed);
		$old_color = $color;
		if (empty($color)) $color = yogastudio_storage_get_array('sc_skills_data', 'color');
		if (empty($color)) $color = yogastudio_get_scheme_color('text_dark', $color);
		if (empty($bg_color)) $bg_color = yogastudio_storage_get_array('sc_skills_data', 'bg_color');
		if (empty($bg_color)) $bg_color = yogastudio_get_scheme_color('text_dark', $bg_color);
		if (empty($border_color)) $border_color = yogastudio_storage_get_array('sc_skills_data', 'border_color');
		if (empty($border_color)) $border_color = yogastudio_get_scheme_color('bd_color', $border_color);;
     if (empty($style)) $style = yogastudio_storage_get_array('sc_skills_data', 'style');
		$title_block = '<div class="sc_skills_info"><div class="sc_skills_label" '. (!empty($color) ? ' style="color:' .esc_attr($color).';"' : '').'>' . ($title) . '</div></div>';
		$style = max(1, min(4, $style));
		$output = '';
		if (yogastudio_storage_get_array('sc_skills_data', 'type') == 'arc' || (yogastudio_storage_get_array('sc_skills_data', 'type') == 'pie' && yogastudio_param_is_on(yogastudio_storage_get_array('sc_skills_data', 'pie_compact')))) {
			if (yogastudio_storage_get_array('sc_skills_data', 'type') == 'arc' && empty($old_color)) {
				$rgb = yogastudio_hex2rgb($color);
				$color = 'rgba('.(int)$rgb['r'].','.(int)$rgb['g'].','.(int)$rgb['b'].','.(1 - 0.1*(yogastudio_storage_get_array('sc_skills_data', 'counter')-1)).')';
			}
            
            yogastudio_storage_concat_array('sc_skills_data', 'legend',
            	'<div class="sc_skills_legend_item"><span class="sc_skills_legend_marker" style="background-color:'.esc_attr($bg_color).'"></span><span class="sc_skills_legend_title">' . ($title) . '</span><span class="sc_skills_legend_value">' . ($value) . '</span></div>');
            
            yogastudio_storage_concat_array('sc_skills_data', 'data', 
            	'<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
				. ' class="'.esc_attr(yogastudio_storage_get_array('sc_skills_data', 'type')).'"'
				. (yogastudio_storage_get_array('sc_skills_data', 'type')=='pie'
					? ( ' data-start="'.esc_attr($start).'"'
						. ' data-stop="'.esc_attr($stop).'"'
						. ' data-step="'.esc_attr($step).'"'
						. ' data-steps="'.esc_attr($steps).'"'
						. ' data-max="'.esc_attr(yogastudio_storage_get_array('sc_skills_data', 'max')).'"'
						...
						. ' data-cutout="'.esc_attr(yogastudio_storage_get_array('sc_skills_data', 'pie_cutout')).'"'
			...
			$output .= (yogastudio_storage_get_array('sc_skills_data', 'columns') > 0
							? '<div class="sc_skills_column column-1_'.esc_attr(yogastudio_storage_get_array('sc_skills_data', 'columns')).'">'
							: '')
					. (yogastudio_storage_get_array('sc_skills_data', 'type')=='bar' && yogastudio_storage_get_array('sc_skills_data', 'dir')=='horizontal' ? $title_block : '')
					. '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
						. ' class="sc_skills_item' . ($style ? ' sc_skills_style_'.esc_attr($style) : '') 
							. (!empty($class) ? ' '.esc_attr($class) : '')
							. (yogastudio_storage_get_array('sc_skills_data', 'counter') % 2 == 1 ? ' odd' : ' even')
							. (yogastudio_storage_get_array('sc_skills_data', 'counter') == 1 ? ' first' : '')
							. '"'
						. (yogastudio_storage_get_array('sc_skills_data', 'height') !='' || $css
							? ' style="' 
								. (yogastudio_storage_get_array('sc_skills_data', 'height') !=''
										? 'height: '.esc_attr(yogastudio_storage_get_array('sc_skills_data', 'height')).';'
										: '')
								. ($css) 
								. '"' 
							: '')
            ...
            if (in_array(yogastudio_storage_get_array('sc_skills_data', 'type'), array('bar', 'counter'))) {
				$output .= '<div class="sc_skills_count"' . (yogastudio_storage_get_array('sc_skills_data', 'type') =='bar' && $color ? ' style="background-color:' . esc_attr($bg_color) . '; color:' . esc_attr($color) . '; border-color:' . esc_attr($border_color) . '"' : '') . '>'
							. '</div>'
							. '<div class="sc_skills_total"'
								...
								. ($start) . ($ed)
							.'</div>';
			} else if (yogastudio_storage_get_array('sc_skills_data', 'type')=='pie') {
			...
			$output .= 
					  (yogastudio_storage_get_array('sc_skills_data', 'type')=='counter' ? $title_block : '')
					. '</div>'
					. (yogastudio_storage_get_array('sc_skills_data', 'type')=='bar' && yogastudio_storage_get_array('sc_skills_data', 'dir')=='vertical' || yogastudio_storage_get_array('sc_skills_data', 'type') == 'pie' ? $title_block : '')
					. (yogastudio_storage_get_array('sc_skills_data', 'columns') > 0 ? '</div>' : '');
		}
...
if ( !function_exists( 'yogastudio_sc_skills_reg_shortcodes' ) ) {
	...
	function yogastudio_sc_skills_reg_shortcodes() {
		...
		yogastudio_sc_map("trx_skills", array(
			...
			"params" => array(
				...
				"dir" => array(
					...
					"type" => "checklist",
					"dir" => "horizontal",
					"options" => yogastudio_get_sc_param('dir')
				), 
				"style" => array(
		...
		vc_map( array(
			...
			"params" => array(
				...
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(yogastudio_get_sc_param('dir')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "style",
