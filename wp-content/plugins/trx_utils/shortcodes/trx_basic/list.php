...
if (!function_exists('yogastudio_sc_list')) {	
	function yogastudio_sc_list($atts, $content=null){	
		...
		yogastudio_storage_set('sc_list_data', array(
			'counter' => 0,
            'icon' => empty($icon) || yogastudio_param_is_inherit($icon) ? "icon-right" : $icon,
...
if (!function_exists('yogastudio_sc_list_item')) {	
	function yogastudio_sc_list_item($atts, $content=null) {
		...
		yogastudio_storage_inc_array('sc_list_data', 'counter');
		$css .= $color !== '' ? 'color:' . esc_attr($color) .';' : '';
		if (trim($icon) == '' || yogastudio_param_is_inherit($icon)) $icon = yogastudio_storage_get_array('sc_list_data', 'icon');
		if (trim($color) == '' || yogastudio_param_is_inherit($icon_color)) $icon_color = yogastudio_storage_get_array('sc_list_data', 'icon_color');
		$content = do_shortcode($content);
		if (empty($content)) $content = $title;
		$output = '<li' . ($id ? ' id="'.esc_attr($id).'"' : '') 
			. ' class="sc_list_item' 
			. (!empty($class) ? ' '.esc_attr($class) : '')
			. (yogastudio_storage_get_array('sc_list_data', 'counter') % 2 == 1 ? ' odd' : ' even')
			. (yogastudio_storage_get_array('sc_list_data', 'counter') == 1 ? ' first' : '')
			. '"' 
			. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
			. ($title ? ' title="'.esc_attr($title).'"' : '') 
			. '>' 
			. (!empty($link) ? '<a href="'.esc_url($link).'"' . (!empty($target) ? ' target="'.esc_attr($target).'"' : '') . '>' : '')
			. (yogastudio_storage_get_array('sc_list_data', 'style')=='iconed' && $icon!='' ? '<span class="sc_list_icon '.esc_attr($icon).'"'.($icon_color !== '' ? ' style="color:'.esc_attr($icon_color).';"' : '').'></span>' : '')
			. trim($content)
			. (!empty($link) ? '</a>': '')
			. '</li>';
...
if ( !function_exists( 'yogastudio_sc_list_reg_shortcodes' ) ) {
	...
	function yogastudio_sc_list_reg_shortcodes() {
		...
		yogastudio_sc_map("trx_list", array(
			...
			"params" => array(
				"style" => array(
					...
					"value" => "ul",
					"type" => "checklist",
					"options" => yogastudio_get_sc_param('list_styles')
					),
				"underlined" => array(
					"title" => __("Line for li", 'trx_utils'),
					"desc" => __("Add line for li", 'trx_utils'),
					"type" => "checkbox"
					),
				"color" => array(
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"admin_label" => true,
					"value" => array_flip(yogastudio_get_sc_param('list_styles')),
					"type" => "dropdown"
					),
				array(
					"param_name" => "underlined",
					"heading" => __("Underline", 'trx_utils'),
					"description" => __("Underline for li", 'trx_utils'),
					"type" => "checkbox"
					),
				array(
					"param_name" => "color",
