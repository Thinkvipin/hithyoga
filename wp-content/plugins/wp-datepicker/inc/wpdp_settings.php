...
// Save the field values
	...
	$wp_datepicker_readonly = get_option( 'wp_datepicker_readonly', true);
...
<input type="radio" name="wp_datepicker_wpadmin" id="wp_datepicker_wpadmin_yes" value="1" <?php checked($wp_datepicker_wpadmin); ?> /><label for="wp_datepicker_wpadmin_yes"><?php _e('Enable', 'wp-datepicker'); ?></label>
<input type="radio" name="wp_datepicker_wpadmin" id="wp_datepicker_wpadmin_no" value="0" <?php checked(!$wp_datepicker_wpadmin); ?> /><label for="wp_datepicker_wpadmin_no"><?php _e('Disable', 'wp-datepicker'); ?></label> 
...
<input type="radio" name="wp_datepicker_readonly" id="wp_datepicker_readonly_yes" value="1" <?php checked($wp_datepicker_readonly); ?> /><label for="wp_datepicker_readonly_yes"><?php _e('Read-only', 'wp-datepicker'); ?></label>
<input type="radio" name="wp_datepicker_readonly" id="wp_datepicker_readonly_no" value="0" <?php checked(!$wp_datepicker_readonly); ?> /><label for="wp_datepicker_readonly_no"><?php _e('Editable', 'wp-datepicker'); ?></label>
</div>


<div class="wp_datepicker_months">
<label for=""><?php _e( 'Weekends?', 'wp-datepicker' ); ?></label>
<input type="radio" name="wp_datepicker_weekends" id="wp_datepicker_weekends_yes" value="0" <?php checked(!$wp_datepicker_weekends); ?> /><label for="wp_datepicker_weekends_yes"><?php _e('Enable', 'wp-datepicker'); ?></label>
<input type="radio" name="wp_datepicker_weekends" id="wp_datepicker_weekends_no" value="1" <?php checked($wp_datepicker_weekends); ?> /><label for="wp_datepicker_weekends_no"><?php _e('Disable', 'wp-datepicker'); ?></label>
...
<textarea name="wp_datepicker_beforeShowDay" id="wp_datepicker_beforeShowDay" placeholder="<?php _e('Insert your custom code here for beforeShowDay'); ?>"><?php echo $wp_datepicker_beforeShowDay; ?></textarea><span style='font-size: 500px;color: rgba(255,255,255,0.8);position: absolute;top: 108px'>&#8624;</span><br /><br />
<small><?php echo __( 'e.g.', 'wp-datepicker').' '.__('Should enable first Thursday Only?', 'wp-datepicker').'<br /><pre>function (date) { <br />var day = date.getDay(); <br />return [(day == 4) && date.getDate()<8];  <br />}</pre><br />'.__('It will disable every date except first thursday of each month.', 'wp-datepicker').'<br />'.__('Note: It will override weekends functionality.', 'wp-datepicker' ); ?></small><br />
<br /><br />
<small><?php echo __( 'e.g.', 'wp-datepicker').' '.__('Should disable Sunday & Monday?', 'wp-datepicker').'<br /><pre><br />function (date) { var day = date.getDay(); return [(day != 0 && day != 1)]; }<br /><br /></pre><br />'.__('It will disable every Sunday & Monday each month.', 'wp-datepicker').'<br />'.__('Note: It will override weekends functionality.', 'wp-datepicker' ); ?></small><br />
</div>
</div>
<?php } ?>
...
<input type="radio" name="wp_datepicker_months" id="wp_datepicker_months_yes" value="1" <?php checked($wp_datepicker_months); ?> /><label for="wp_datepicker_months_yes"><?php _e('Short', 'wp-datepicker'); ?></label>
<input type="radio" name="wp_datepicker_months" id="wp_datepicker_months_no" value="0" <?php checked(!$wp_datepicker_months); ?> /><label for="wp_datepicker_months_no"><?php _e('Full', 'wp-datepicker'); ?></label>
<small><?php echo __( 'e.g.', 'wp-datepicker').' Sep '.__('or September?', 'wp-datepicker' ); ?></small>
</div>



<?php if($wpdp_pro && function_exists('wpdp_pro_settings')){ wpdp_pro_settings(); }else{ wpdp_free_settings(); } ?>
