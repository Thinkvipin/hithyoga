...
jQuery(document).ready(function($){
	...
	$('.wpdp_cg_btn').click(function(){
		$('.wpdp_cg').fadeToggle();
		$(this).toggleClass('clicked');
	});
	
	$('.wpdp_inline_div').on('click', 'img', function(){
		$(this).toggleClass('clicked');
	...
	$('.wpdp_pro_calendars li a').on('click', function(){
		$('input[name="wpdp_options[use_custom_style1]"]').val($(this).data('name'));
