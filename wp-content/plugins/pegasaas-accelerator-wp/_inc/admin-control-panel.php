...
<div class='pegasaas-header <?php if (PegasaasAccelerator::$settings['status'] < 1) { ?>pegasaas-disabled<?php } ?>'>
	...
	<?php } else {
	  ...
	  ?>
    <img id="plugin-logo" src='<?php echo PEGASAAS_ACCELERATOR_URL; ?>assets/images/pegasaas-accelerator-horizontal-logo<?php echo $logo_modifier; ?><?php echo ($pegasaas->is_pro_edition() ? "-dev" :  ""); ?>.png' style='margin-left: 00px;'/>
...
function manage_interval_time() {
	...
	if (total_requests_this_page_load == 20) {
		current_interval = current_interval * 2;
	} else if (total_requests_this_page_load == 50) {
		current_interval = current_interval * 2;
	} else if (total_requests_this_page_load >= 100) {
...
function init_bindings() {
	...
	jQuery("a.initiate-rescan-pagespeed-benchmark").bind("click", function(e) {
		...
		jQuery.post(ajaxurl,
					{ 'action': 'pegasaas_request_pagespeed_benchmark_score', 'api_key': jQuery("#pegasaas-api-key").val(), 'resource_id': resource_id},
					function(data) {
						total_requests_this_page_load++;
...
function toggle_display_mode(element) {
	...
	if (jQuery(element).val() == 0) {
		...
		jQuery("#plugin-logo").attr("src", PEGASAAS_ACCELERATOR_URL + "assets/images/pegasaas-accelerator-horizontal-logo-light.png");
		...
		jQuery("head").append("<link id='pegasaas-accelerator-admin-styles-light-css' rel='stylesheet' href='" + PEGASAAS_ACCELERATOR_URL + "/assets/css/light.css' type='text/css'>");
		jQuery("#plugin-logo").attr("src", PEGASAAS_ACCELERATOR_URL + "assets/images/pegasaas-accelerator-horizontal-logo-light.png");
		...
		jQuery("head").append("<link id='pegasaas-accelerator-admin-styles-plain-css' rel='stylesheet' href='" + PEGASAAS_ACCELERATOR_URL + "/assets/css/plain.css' type='text/css'>");
		jQuery("#plugin-logo").attr("src", PEGASAAS_ACCELERATOR_URL + "assets/images/pegasaas-accelerator-horizontal-logo-dark.png");
