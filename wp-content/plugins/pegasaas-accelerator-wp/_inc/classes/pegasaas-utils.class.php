   ...
   /**
	...
	function update_object_meta($object_path, $meta_type, $meta_data) {
		...
		if ($meta_type == "accelerator_overrides") {
			...
			$pegasaas->db->update_record("pegasaas_page_config", array("resource_id" => $object_path, "settings" => $data));
	...
	static function get_permalink() {		
		...
		if (isset($request_data['query'])) {
		...
		} else if (self::wpml_multi_domains_active()) {
			...
			if ($website_address != $this_address){
	...
	static function get_object_id($id = NULL) {
		...
		//	$object_id = str_replace($location, "", $object_id);
			...
			// assign the resourc_id as /mydomain.com/ in the event that this is a multi-domain installation
			...
			if ($path == "/" && strstr($url_data['query'], "page_id=")) {
				...
				$slug = str_replace($pegasaas->get_home_url('', 'https'), "", $permalink);
				$slug = str_replace($pegasaas->get_home_url('', 'http'), "", $slug);
	...
	function get_semaphore($var) {
		return get_option("pegasaas_semaphore_lock__{$var}", false);
	}
	...
	function rotate_log_files() {
		...
		if (file_exists($log_files_folder."log.5.txt")) {
			@unlink($log_files_folder."log.5.txt");
		}
		if (file_exists($log_files_folder."log-critical.5.txt")) {
			@unlink($log_files_folder."log-critical.5.txt");
		}
		
		for ($log_number = 5; $log_number--; $log_number > 0) {
			$new_number = $log_number+1;
			if (file_exists($log_files_folder."log.{$log_number}.txt")) {
			
				rename($log_files_folder."log.{$log_number}.txt", $log_files_folder."log.{$new_number}.txt");
			}
			if (file_exists($log_files_folder."log-critical.{$log_number}.txt")) {
			
				rename($log_files_folder."log-critical.{$log_number}.txt", $log_files_folder."log-critical.{$new_number}.txt");
			}			
		}
		@rename(PEGASAAS_ACCELERATOR_DIR."/log.txt", $log_files_folder."log.1.txt");	
		@rename(PEGASAAS_ACCELERATOR_DIR."/log-critical.txt", $log_files_folder."log-critical.1.txt");	
	}
	
	function log($message, $subsystem = "") {
		global $pegasaas;
		...
		if ($subsystem == "") {
			...
			//$log_this = true;
			//$subsystem = $subsystem."-".PegasaasAccelerator::$settings['settings']['logging']["log_{$subsystem}"];
		}
		
		if ($subsystem == "ERROR" || $subsystem == "CRITICAL" || (isset(PegasaasAccelerator::$settings['settings']['logging']['status']) && PegasaasAccelerator::$settings['settings']['logging']['status'] == 1 && $log_this)) {
			if ($subsystem == "CRITICAL" || $subsystem == "data_structures") {
				$logfile = PEGASAAS_ACCELERATOR_DIR."/log-critical.txt";
			} else {
			...
			$message 	= "[{$time} / {$pegasaas->instance} / {$_SERVER['REMOTE_ADDR']}] **{$subsystem}** {$message}\n";
			fwrite($fp, $message);
			fclose($fp);
		}
	}
	...
	function semaphore($var, $ms = 5000) {
		if ($var == "testing_semaphore") {
			$debug = true;
			$start_time = time();
		}
		...
		$ms_interval 			= 500;
		$max_intervals 			= $ms / $ms_interval;
		
		$interval 				= 0;
		
		while($semaphore = get_option("pegasaas_semaphore_lock__{$var}", false) && $interval++ < $max_intervals) {
			usleep(5000); 
			if ($debug) {
				print "<pre class='admin'>encountered lock? for {$var}  -- ".$this->get_microtime()."</pre>";
			}
		}
		
		// if we reach the limit that of time requested to wait, then return a failure status
		...
		$this->log("Semaphore ($var) LOCKED by {$debug_backtrace_string}", "semaphores");
					if ($debug) {
				print "<pre class='admin'>semaphore locked for {$var}</pre>";
			}
	...
	function release_semaphore($var) {
		...
		$semaphores = get_option("pegasaas_semaphores", array());
		unset($semaphores["$var"]);
		if (array_key_exists($var, $semaphores)) {
			$this->log("Unsetting Semphore $var -- still existing in array", "semaphores");
		} else {
			$this->log("Unsetting Semphore $var -- cleared from array ".sizeof($semaphores), "semaphores");
		}
	...
	function touch_url($url, $args = array(), $proxy_this_request = false) {
		...
		if (!$this->execution_time_ok()) {
			//print "execution time not ok {$url}<br>";
		...
		if (is_a($output, "WP_Error")) {
			...
			if ($http_response) { 
				...
				if ($response->redirects > 0 && $response_url_path != $requested_url_path) {
	...
	function strip_ignored_query_arguments($query_string) {
		...
		foreach ($args as $name => $value) {
			if (in_array($name, $ignored_query_arguments)) {
				unset($args["$name"]);
			}
		}
...
*/
	...
	function console_log($output) {
		...
		print "<script>console.log('{$pegasaas->instance} {$output}: {$total_time}');</script>";
	...
	static function get_all_pages_and_posts($include_pegasaas_deferred_js = false) {
		global $pegasaas;  
		...
		if (isset(PegasaasUtils::$all_pages_and_posts) && PegasaasUtils::$all_pages_and_posts && sizeof(PegasaasUtils::$all_pages_and_posts) > 0) {
			return PegasaasUtils::$all_pages_and_posts;
		}
	
		// check to see if the array exists within the session
		PegasaasUtils::$all_pages_and_posts = $pegasaas->data_storage->get("all_pages_and_posts");
	if (false && $debug) {
		print "<pre class='admin'>";
		var_dump(PegasaasUtils::$all_pages_and_posts);
		print "</pre>";
	}
				...
				foreach ($all_posts_array as $x => $page) {
					foreach ($wpml_domains as $lang => $domain) {
						if ($domain == "default") {
							...
							//print "XXX".$page_on_front_id."<br>\n";
						}
						
						if ($page->ID == $page_on_front_id) { 
							if ($domain == "default") {
								//print "have default lang\n";
	...
	//	print "<pre class='admin'>";
		...
		$pegasaas->data_storage->set("all_pages_and_posts", $all_posts_array);
