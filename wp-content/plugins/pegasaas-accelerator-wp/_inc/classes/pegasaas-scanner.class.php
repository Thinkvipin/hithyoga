...
class PegasaasScanner {
	...
	function maybe_request_page_scan($resource_id, $priority_flag) {
		global $pegasaas;
		...
		if (isset(PegasaasAccelerator::$settings['settings']['auto_rescan_webperf']) && PegasaasAccelerator::$settings['settings']['auto_rescan_webperf']['status'] == 1) {
			$existing_queued_requests = $pegasaas->db->get_results("pegasaas_api_request", array("request_type" => "pagespeed", "resource_id" => $resource_id));
			$total_existing_queued_requests = $pegasaas->db->get_results("pegasaas_api_request", array("request_type" => "pagespeed"));
	...
	function submit_benchmark_requests() {
		...
		if ($pegasaas->utils->semaphore("submit_benchmark_requests")) {
			...
			$existing_benchmark_request_records = $pegasaas->db->get_results("pegasaas_api_request", array("request_type" => "pagespeed-benchmark"));
		...
		if( sizeof($pages_to_submit) > 0) { 
			...
			$response = $pegasaas->api->post($post_fields, array("timeout" => PEGASAAS_ACCELERATOR_API_TIMEOUT, "blocking" => false));
	...
	function pegasaas_request_image_data($die = true) { 
		...
		if ($_POST['api_key'] == PegasaasAccelerator::$settings['api_key']) {
			if ($_POST['resource_id'] == "") {
				...
				$response = $this->api->post($post_fields, array("timeout" => PEGASAAS_ACCELERATOR_API_TIMEOUT));
				...
				if ($data['api_error'] != "") {
					...
					// request failed for some reason
					 if ($data['image_data_request_status'] == 0) {
						...
						if ($pegasaas->utils->semaphore("pegasaas_accelerator_image_data_requests")) { 
	...
	function pegasaas_request_pagespeed_benchmark_score($die = true) {
		...
		if ($_POST['api_key'] == PegasaasAccelerator::$settings['api_key']) {
			if ($_POST['resource_id'] == "") {
				...
				// send request and save response to variable
				$response = $pegasaas->api->post($post_fields, array("timeout" => PEGASAAS_ACCELERATOR_API_TIMEOUT));
	...
	function clear_pagespeed_requests() {
		...
		if ($pegasaas->utils->semaphore("pegasaas_accelerator_pagespeed_score_requests")) {
			$pegasaas->utils->log("About to instruct API to dump all pagespeed-requests");
			$response = $pegasaas->api->post(array("command" => "cancel-all-pagespeed-requests"));
			
			delete_option("pegasaas_accelerator_pagespeed_score_request_tokens");
			delete_option("pegasaas_accelerator_pagespeed_score_requests");	
			$pegasaas->utils->release_semaphore("pegasaas_accelerator_pagespeed_score_requests");
			$pegasaas->utils->log("PageSpeed requests cleared");
			$pegasaas->db->delete("pegasaas_api_request", array("request_type" => "pagespeed"));
		}
		/*
		if ($pegasaas->utils->semaphore("pegasaas_accelerator_pagespeed_benchmark_score_requests")) {
			$pegasaas->api->post(array("command" => "cancel-all-benchmark-pagespeed-requests"));
			
			delete_option("pegasaas_accelerator_pagespeed_benchmark_score_request_tokens");
			delete_option("pegasaas_accelerator_pagespeed_benchmark_score_requests");	
			$pegasaas->utils->release_semaphore("pegasaas_accelerator_pagespeed_benchmark_score_requests");
		}
		*/
		//print "done clearing";
	}
	...
	function clear_pagespeed_benchmark_requests() {
		...
		*/
		if ($pegasaas->utils->semaphore("pegasaas_accelerator_pagespeed_benchmark_score_requests")) {
			$pegasaas->utils->log("About to instruct API to dump all benchmark pagespeed-requests");
			$pegasaas->api->post(array("command" => "cancel-all-benchmark-pagespeed-requests"));
			
			delete_option("pegasaas_accelerator_pagespeed_benchmark_score_request_tokens");
			delete_option("pegasaas_accelerator_pagespeed_benchmark_score_requests");	
			$pegasaas->utils->release_semaphore("pegasaas_accelerator_pagespeed_benchmark_score_requests");
			$pegasaas->utils->log("Benchmark PageSpeed requests cleared");
			$pegasaas->db->delete("pegasaas_api_request", array("request_type" => "pagespeed-benchmark"));
...
/*
	...
	function get_scanned_objects($order_by = "score_desc", $object_type = "") {
		...
		if ($pegasaas->interface->results_post_type == "") {
			...
			$posts_array		= $pegasaas->utils->get_all_posts($pegasaas->interface->results_post_type, $pegasaas->interface->results_search_filter);
		...
		foreach ($posts_array as $post) {
			...
			//	print "is accerated\n";
				...
				//print "\n resource id (2): {$post_data[resource_id]}\n";
	...
	function pegasaas_fetch_pagespeed_last_scan($resource_id, $use_cache = false) {
		...
		if ($pagespeed_scan_record) {
			...
			$speed_index_mobile = number_format($post_data['meta']['mobile']['lab_data']['speed-index']['displayValue'], 1, '.', '');
			$speed_index_desktop = number_format($post_data['meta']['desktop']['lab_data']['speed-index']['displayValue'], 1, '.', '');
			...
			$interactive_mobile = number_format($post_data['meta']['mobile']['lab_data']['interactive']['displayValue'], 1, '.', '');
			$interactive_desktop = number_format($post_data['meta']['desktop']['lab_data']['interactive']['displayValue'], 1, '.', '');
			...
			// condition fmp
			$fmp_mobile = number_format($post_data['meta']['mobile']['lab_data']['first-meaningful-paint']['displayValue'], 1, '.', '');
			$fmp_desktop = number_format($post_data['meta']['desktop']['lab_data']['first-meaningful-paint']['displayValue'], 1, '.', '');
			...
			// condition fcp
			$fcp_mobile = number_format($post_data['meta']['mobile']['lab_data']['first-contentful-paint']['displayValue'], 1, '.', '');
			$fcp_desktop = number_format($post_data['meta']['desktop']['lab_data']['first-contentful-paint']['displayValue'], 1, '.', '');
			...
			// condition lcp
			$lcp_mobile = number_format($post_data['meta']['mobile']['lab_data']['largest-contentful-paint']['displayValue'], 1, '.', '');
			$lcp_desktop = number_format($post_data['meta']['desktop']['lab_data']['largest-contentful-paint']['displayValue'], 1, '.', '');
			if ($fcp_mobile != 0.0 && $fcp_mobile != "") {
			...
			// condition tbt
			$tbt_mobile = number_format($post_data['meta']['mobile']['lab_data']['total-blocking-time']['displayValue'], 1, '.', '');
			$tbt_desktop = number_format($post_data['meta']['desktop']['lab_data']['total-blocking-time']['displayValue'], 1, '.', '');
			if ($fcp_mobile != 0.0 && $fcp_mobile != "") {
			...
			// condition cls
			$cls_mobile = number_format($post_data['meta']['mobile']['lab_data']['cumulative-layout-shift']['displayValue'], 1, '.', '');
			$cls_desktop = number_format($post_data['meta']['desktop']['lab_data']['cumulative-layout-shift']['displayValue'], 1, '.', '');
			if ($fcp_mobile != 0.0 && $fcp_mobile != "") {
			...
			$ttfb_mobile = @number_format($ttfb_mobile, 0);
			...
			$ttfb_desktop = number_format($ttfb_mobile, 0);
	...
	function pegasaas_process_pagespeed_benchmark_score_request() {
		...
		if ($_POST['api_key'] == PegasaasAccelerator::$settings['api_key']) {
			//if ($pegasaas->utils->semaphore("pegasaas_process_pagespeed_benchmark_score_request")) {
				$data = json_decode(stripslashes($_POST['data']), true);
...
/*Where adam left off*/
	...
	function get_site_pages_accelerated_data() {
		...
		$pages_and_posts = PegasaasUtils::get_all_pages_and_posts();
		...
		update_option("pegasaas_accelerated_pages", $accelerated_pages);
	...
	function get_pages_with_scores($type, $resource_id = "") { 
		...
		if ($this->data_cache["pegasaas_performance_scan_{$type}"]) {
			...
			if (sizeof($this->data_cache) == 0) {
				...
				$size_of_typical_scan = 55000 * 2; // scan + benchmark
		...
		// first attempt to see if there is an object in the class
			...
			//	print "less than 15\n";
				if ($performance_data['stale_records'] > 0 && $performance_data['last_updated'] < time() - $fifteen_minutes) {
					...
					//print $performance_data['last_updated']."\n";
					//print time()."\n";
					//print $performance_data['last_updated'] < time() - $fifteen_minutes."<br>";
					//var_dump($performance_data['performance_data']);
					$this->performance_data = $performance_data['performance_data'];
	...
	//	print "Okay";
		...
		*/
		$performance_data_temp = array(
			...
			"cumulative-layout-shift"	=> array("title" => "Cumulative Layout Shift", 	"unit" => "",	"abbr" => "CLS", 	"percent" => 0, "value" => 0, "before" => 0, "change" => 0, "description" => "CLS is the unexpected shifting of web page elements while the page is still downloading. The kinds of elements that tend to cause shift are fonts, images, videos, contact forms, buttons and other kinds of content. CLS is often caused by regions of the page being rendered on-page-load by JavaScript."),
			);
		...
		$performance_metrics['cumulative-layout-shift'] = array("fast" => 0.1, "slow" => 0.25, 'median' => 0.1, 'falloff' => 2.9);
	...
	function sort_scan_items_by_page_importance($a, $b) {
		...
		$ascore = substr_count($a['slug'], '/');
		$bscore = substr_count($b['slug'], '/');
	...
	function submit_scan_request() {
		...
		if ($pegasaas->utils->semaphore("submit_scan_requests")) {
			$allows_gpsi_calls = $this->caching_allows_gpsi_calls();
			...
			if (is_array($posts_array)) {
				...
				if ($debug) { print "Ajusted Size of Posts/Pages Array: ".sizeof($posts_array)."\n"; }
			...
			foreach ($enabled_posts as $post) {	
				...
				if ($post_pagespeed_history) {
					...
					if (!$existing_requests["$resource_id"] ) {	
						if ($debug) { 
						  print "Apparently Request Does Not Exist for {$resource_id}\n";
						}
						$request_this_resource = true;
					} else {
						if ($debug) {
