...
//return; 
		...
		if ($this->is_pro_edition()) { 
			self::$post_type_cpcss 	= $this->get_critical_css_records("post_type");
			self::$page_level_cpcss = $this->get_critical_css_records("custom");
		}
		...
		add_action( 'pegasaas_auto_clear_page_cache_daily', 	array($this, "auto_clear_page_cache_daily"));
		add_action( 'pegasaas_auto_clear_page_cache_biweekly', 	array($this, "auto_clear_page_cache_biweekly"));
		add_action( 'pegasaas_auto_clear_page_cache_weekly', 	array($this, "auto_clear_page_cache_weekly"));
		add_action( 'pegasaas_auto_clear_page_cache_monthly', 	array($this, "auto_clear_page_cache_monthly"));
		...
		add_filter('cron_schedules', array($this->utils, 'add_custom_cron_intervals'), 10, 1);
		...
		add_action('shutdown', array($this, 'shutdown'), -99999); 
		
		add_action('the_post', array($this, 'set_post'));
		add_action('template_redirect', array($this, 'handle_404'));
		
		
		//add_action('pre_handle_404', array($this, 'handle_404'));
		//add_action('pre_handle_404', array($this, 'pre_handle_404'));
	...
	function auto_crawl() {
		...
		if (PegasaasAccelerator::$settings['settings']['auto_crawl']['status'] == 0) {
			...
			$max_interval = 1; // seconds between requests
						...
						if ($this_page_caching_status) {
							...
							if (!$cache_data_map["{$url}"]) {							
								...
								if ($page_cache_request_count < $maximum_submits_per_invocation) {
									//if ($page_cache_request_count > 0) {
										//sleep($max_interval);
									//}
									$page_cache_request_count++;
									$this->utils->touch_url($url);
									$this->utils->log("Cache for {$url} created" , "auto_crawl");
	...
	function pegasaas_error_handler($errno, $errstr, $errfile, $errline ) {
		...
		} else if (array_key_exists($typestr, $always_log)) {
	...
	function is_standard() {
		return PEGASAAS_ACCELERATOR_TYPE == "standard";
	}
		...
		if (!is_admin() && $this->output_buffer_mode == "functionless") {
			...
			if ($this->is_on_excluded_page()) {
				...
				// The fallback method is to wrap the sanitize output with a "fatal_error_handler" output buffer trap function
					...
					print $buffer;
					if (!defined('WP_CLI')) {
						print "<!-- pegasaas://accelerator -- fatal error detected, sanitize output aborted -->";
					}
				}
			}
	...
	function fatal_error_handler($buffer) {
		$error = error_get_last();
    	if ($error['type'] == 1){
			$message = 'E_ERROR: '.$error['message'].' in '.$error['file'].' on line #'.$error['line'];
	...
	function init() {
		...
		if (!PegasaasAccelerator::is_amp_endpoint()) {
		...
		*/
		...
		if (is_admin()) {
			...
			if (get_option("pegasaas_accelerator_dismiss_review_prompt", 0) == 0) { 
	...
	function auto_clear_page_cache_daily() {
		...
		$this->utils->log("CRON: Auto Clear Cache DAILY (start)", "auto_clear_page_cache");
		PegasaasUtils::get_object_meta("/", "accelerator_overrides", true); // just initialize the object if it isn't initialized aleady
	
		foreach ($this->utils->data_cache["accelerator_overrides"] as $resource_id => $page_level_settings) {
			if ($page_level_settings["accelerated"] == true || $page_level_settings["accelerated"] > 0) {
				if ($page_level_settings["auto_clear_page_cache"] == "daily") {
					...
					$this->cache->clear_cache($resource_id);
				}
			}
		}
		$this->utils->log("CRON: Auto Clear Cache DAILY (end)", "auto_clear_page_cache");
	}	
	

	function auto_clear_page_cache_biweekly() {
		if (PegasaasAccelerator::$settings['settings']['auto_clear_page_cache']['status'] == 3) {
			$this->utils->log("CRON: Auto Clear Cache BI-WEEKLY", "auto_clear_page_cache");
	...
	function auto_clear_page_cache_weekly() {
		if (PegasaasAccelerator::$settings['settings']['auto_clear_page_cache']['status'] == 2) {
			$this->utils->log("CRON: Auto Clear Cache WEEKLY", "auto_clear_page_cache");
			$this->cache->clear_cache();
	...
	function auto_clear_page_cache_monthly() {
		...
		if (PegasaasAccelerator::$settings['settings']['auto_clear_page_cache']['status'] == 1) {
			$this->utils->log("CRON: Auto Clear Cache MONTHLY", "auto_clear_page_cache");
			$this->cache->clear_cache();
		}
	}
	...
	function pegasaas_clear_optimization_request() {
		...
		if ($_POST['api_key'] == PegasaasAccelerator::$settings['api_key']) {
			...
			$response = $this->api->post(array("command" => "cancel-optimization-request", 
											   "url" => $url), 
										 array('timeout' => PEGASAAS_ACCELERATOR_API_TIMEOUT, 
											   'blocking' => false)); 
			print 1;
		} else {
			print -1;
	...
	function pegasaas_process_optimization_request() {
		...
		if ($_POST['api_key'] == PegasaasAccelerator::$settings['api_key']) {
			...
			print "Endpoint: {$_SERVER['SERVER_ADDR']}<br>";
			...
			if ($resource_id != "") { 
				...
				$this->scanner->maybe_request_page_scan($resource_id, $priority_flag);
	...
	function register_cron_events() {
		...
		if (!wp_next_scheduled('pegasaas_auto_crawl')) {
			wp_schedule_event(time() + 120, 'five_minutes', "pegasaas_auto_crawl");
		}	
	...
	function auto_accelerate_pages($reset = false) {
		...
		if ($this->utils->semaphore("auto_accelerate_pages")) {
			...
			if ($total_accelerated < PegasaasAccelerator::$settings['limits']['maximum_page_accelerations_available']) {
				...
				if ($_POST['acceleration-type'] == "page") {
					...
					PegasaasUtils::$all_pages_and_posts = array();
					$this->set_home_page_accelerated(1, true);
					$this->set_categories_accelerated(1, true);
					$posts_array 		= $this->utils->get_all_pages_and_posts();
				...
				if ($debug) {
					var_dump($_POST['enable_acceleration_on']);
				}
					...
					// if we have exceeded the allowable accelerations for this subscription, set the acceleration status to 0
						...
						if (array_key_exists($resource_id, $accelerated_pages)) {
							...
							if ($post_acceleration_status == true) {
								...
								$total_accelerated++;
								$accelerated_pages["{$resource_id}"] = 1;
								if (!is_object($this->utils->all_pages_and_posts["$pid"])) {
									$this->utils->all_pages_and_posts["$pid"] = new stdClass();
								}
								$this->utils->all_pages_and_posts["$pid"]->accelerated = true;
		...
		if (strstr($url, "wp-admin")) { 
  		...
  		if (PegasaasAccelerator::$settings['api_key'] != "" || $test_submission) {
			...
			if (!$this->db->has_record("pegasaas_api_request", array("resource_id" => $resource_id, "request_type" => "optimization-request"))) {
				...
				$response = json_decode($this->api->post($post_fields, array('timeout' => PEGASAAS_ACCELERATOR_API_OPTIMIZATION_TIMEOUT, 'blocking' => true)), true);
	...
	function record_optimization_request($resource_id) {
		...
		$request_id = $pegasaas->db->add_record("pegasaas_api_request", array("resource_id" => $resource_id, "request_type" => "optimization-request", "time" => date("Y-m-d H:i:s")));
		return $request_id;
	}
  ...
  "  BrowserMatch \bMSIE !no-gzip !gzip-only-text/html\n".
	...
	function set_home_page_accelerated($enabled, $save_setting = false) {
		...
		//	print "Enabled is actually [{$enabled}]\n";
		}
		$page_level_settings['accelerated'] = $enabled;
	//	print "Setting Page Level Settings Accelerated to ".print_r($enabled, true)."\n";
		$this->utils->update_object_meta($resource_id, "accelerator_overrides", $page_level_settings);
...
<?php
	...
	function clear_queued_requests() {
		$response = $this->api->post(array("command" => "cancel-all-cpcss-requests")); 
		delete_option('pegasaas_pending_critical_css_request');
	}

	function clear_queued_optimization_requests() {
		$response = $this->api->post(array("command" => "cancel-all-optimization-requests"), array("timeout" => PEGASAAS_ACCELERATOR_API_TIMEOUT, "blocking" => false)); 
		//delete_option('pegasaas_pending_critical_css_request');
	...
	function disable_accelerator_for_page($resource_id, $set_accelerated_pages = true) {
		...
		$accelerated_pages = get_option("pegasaas_accelerated_pages");
