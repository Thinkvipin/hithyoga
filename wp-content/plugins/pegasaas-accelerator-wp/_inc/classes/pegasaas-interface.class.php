...
class PegasaasInterface {
	...
	function get_advanced_visible_gauges_class() {
		...
		$count = 8;
		$metrics = array ("ttfb", "fcp", "si", "lcp", "tti", "fci", "tbt", "cls");
		...
		switch ($column_name) {
			...
			case 'pegasaas-accelerator-mobile-speed':
				...
				if (is_array($page_score_data)) {
					...
					$interactive_time = number_format($interactive_time, 1, '.', '');
...
-->
	...
	function pegasaas_fetch_page_metrics() {
		...
		if ($_POST['api_key'] == PegasaasAccelerator::$settings['api_key']) {
			...
			$data['cache_map'] = PegasaasAccelerator::$cache_map;
	...
	function pegasaas_dashboard_settings_update() {
		...
		if ($_POST['api_key'] == PegasaasAccelerator::$settings['api_key']) {
			...
			// - defer unused css -- 0,1,1500,7500
				...
				$pegasaas->set_feature($_POST['args']['f'], $_POST['args']['s']);
				$data['status'] = 4;
			...
			// - "css_cache_busting" and "js_cache_busting" for "cdn"	
				...
				$secondary_status = $pegasaas->set_feature($_POST['args']['f'], 1, array($_POST['args']['a'] => $_POST['args']['s']));
				$data['status'] = 7;
...
<div id="pegasaas-accelerator__page_post_options_sidebar" <?php if ($page_level_configurations['accelerated'] != true) { ?>class='pegasaas-disabled <?php echo $view_mode; ?>'<?php } else { ?>class='<?php echo $view_mode; ?>'<?php } ?>>
