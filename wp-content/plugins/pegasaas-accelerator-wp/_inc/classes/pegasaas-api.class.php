   ...
   /**
	...
	function pegasaas_api_key_check($api_key = "", $die = true) {
		...
		if ($request_key) {
			...
			$response = $this->post(array("api_key" => "request-new-quick-key", 
	...
	function post($post_fields, $args = array()) {
		...
		if ($args['timeout'] == "") {
			$args['timeout'] = PEGASAAS_ACCELERATOR_API_TIMEOUT;
		}
		...
		$response = wp_remote_request(PEGASAAS_API_KEY_SERVER, $args);
		...
		if (is_a($response, "WP_Error")) {
			if ($args['blocking'] == true) {
				...
				$args['timeout'] = PEGASAAS_ACCELERATOR_API_TIMEOUT;
				wp_remote_request(PEGASAAS_API_KEY_SERVER, $args);
		...
		// the response object can be used to get ->url or ->redirects
