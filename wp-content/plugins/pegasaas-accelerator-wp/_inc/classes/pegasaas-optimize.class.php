...
class PegasaasOptimize {
	...
	//	$pegasaas->utils->log("Sanitize 1", "critical");
		$debug 		 = false;
		...
		if (strstr($_SERVER['REQUEST_URI'], "/!/external-css,") !== false) {
			...
			header("HTTP/1.0 201 Query Args Exist"); // required for Kinsta cache
			if ($pegasaas->in_development_mode() && !$bypass_staging_mode) {	
				...
				$buffer = $this->local_optimize($buffer, $save_cache = false);
			}
	...
	//		PegasaasAccelerator::$settings['settings']['defer_render_blocking_js']['override'] = 3;
			...
			header("X-Pegasaas-Message: Caching Disabled");
			header("X-Pegasaas-Reason: User Logged In");
		...
		} else if (function_exists('is_404') && is_404()) {
			header("X-Pegasaas-Message: Caching Disabled");
		...
		if ($pegasaas->in_development_mode() && !$bypass_staging_mode) {
			...
			if (!$pegasaas->cache->save_cached_copy) {
	...
	//	var_dump(PegasaasAccelerator::$settings['settings']['coverage']);
		
		
		/*
		if (!$this->can_request_api_optimization()) {
			global $proxy_buffer;
							$buffer .= PegasaasUtils::html_comment("Local Optimization Only: A");
			$buffer .= $proxy_buffer;
		}
		if ($is_amp) {
			$buffer .= PegasaasUtils::html_comment("Local Optimization Only: B");
		}
		
		$buffer .= PegasaasUtils::html_comment("Local Optimization Only: C");
		*/
	...
	function local_optimize($buffer = "", &$save_cache = false) {
		...
		if (!$this->is_wordlift_api()) {
			...
			//	$website_address = $pegasaas->get_home_url();
				...
				$date_generated   = gmdate("F jS, Y @ H:i", time())." UTC";
