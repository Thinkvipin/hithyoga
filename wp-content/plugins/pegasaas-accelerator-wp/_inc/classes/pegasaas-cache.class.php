...
class PegasaasCache {
	...
	function handle_post_change_state($new_status, $old_status, $post) {
		...
		// no need to clear cache on Woo Commerce orders
		if (isset($post) && isset($post->post_type) && $post->post_type == "shop_order") { 
			return;
		}
	...
	function check_cache() {
		...
		} else if (PegasaasAccelerator::$settings['settings']['accelerated_mobile_pages']['status'] == 1 && $pegasaas->utils->is_mobile_user_agent()) {
			return;
		}
		...
		if ($_SERVER['HTTP_X_PEGASAAS_BYPASS_CACHE'] == 1) {
			...
			$pegasaas->utils->log("Check Cache -- caching type {$caching_type}", "caching");
			 ...
			 if ($caching_type == "file") {
				 ...
				 if ((@$cached_page['file'] == "" || !@file_exists($cached_page['file'])) && !is_user_logged_in()) {
					...
					$this->save_cached_copy = true;
					 ...
					 // do not display cached copy of page if the user is logged in
				   } else if (!is_user_logged_in() || $accelerate_explicitly_on) {
			 ...
			 } else if ($caching_type == "db") {
			 		if ((@$cached_page['html'] == "" || $adjusted_time < strtotime($post_obj->post_modified)) && !is_user_logged_in()) {
						$this->save_cached_copy = true;
					 ...
					 // do not display cached copy of page if the user is logged in
							...
							$when_created = date("Y-m-d H:i:s (T)", $cached_page['when_cached']);
							$cached_page['html'] = $pegasaas->apply_powered_by_message($cached_page['html']);
							print $cached_page['html'];
		if (method_exists($pegasaas, "is_wordlift_api") &&!$pegasaas->is_wordlift_api()) {
								//print "\n<!-- pegasaas://accelerator - displaying cached resource ({$object_id}) // built {$when_created} -->";
								header("X-Pegasaas-Cache-Message: displaying cached resource ({$object_id}) // built {$when_created}");
	...
	function purge_cloudflare($file = "") {
		...
		//print $url;
		$args 		= array("headers" => $headers, "method" => "POST");

		if ($file == "") {
			$args['body'] ='{"purge_everything":true}';
		} else {
			$files = array();
			...
			$args['body'] = json_encode(array("files" => $files));
		}
	...
	function queue_clear_global_cache($cache_type) {
		...
		//$cache_map = get_option("pegasaas_cache_map", array());
	...
	function clear_queued_cache_resources($debug = false) {
		...
		foreach ($records as $record) {
			...
			if ($task_type == "clear_html_cache") {
				...
				$success = $pegasaas->utils->touch_url($resource_id, array("headers" => array("X-Pegasaas-Priority-Optimization" => 2,
		...
		if ($total_remaining < 1) {
			$cache_map = get_option("pegasaas_cache_map");
			print "<Pre>";
			var_dump($cache_map);
			print "</pre>";
			$files = $this->get_list_of_all_local_files("html");
			print "<pre>";
			var_dump ($files);
			print "</pre>";
		}
	...
	function clear_cache($object_id = "") {
		...
		if ($object_id == "") {
			$pegasaas->data_storage->unset("local_file_stats_html");
			$pegasaas->data_storage->unset("local_file_stats_combined.css");
			$pegasaas->data_storage->unset("local_file_stats_deferred-js.js");
			$this->queue_clear_global_cache("all");
		} else {
			$pegasaas->utils->log("About to CLEAR HTML CACHE", "caching");
	...
	function get_list_of_all_local_files($file_type) {
		$files = array();
	...
	function clear_local_resource_cache($file_type = "") {
		...
		$all_files = $this->get_list_of_all_local_files($file_type);
		//var_dump($all_files);
	...
	function clear_html_cache($object_id = "") {
		...
		if ($object_id === "") {
			...
			$pegasaas_cache_map = get_option('pegasaas_cache_map', array());
	...
	function get_local_cache_stats($extension) {
		global $pegasaas;
		
		if ($pegasaas->data_storage->is_valid("local_file_stats_{$extension}")) {
	...
	function save_page_cache($buffer, $object_id = "", $url = "", $temp_cache = false, $css_validation_issue = false, $extended_coverage = false) {
		...
		if ($pegasaas->utils->semaphore("pegasaas_cache_map")) {
			$cached_resource 	= PegasaasUtils::get_object_meta($object_id, "cached_html");
			...
			if ($file_based_caching) {
				...
				if ($pegasaas->utils->semaphore("html_write_cache_{$file_folder_name}")) {
					...
					$pegasaas->utils->release_semaphore("html_write_cache_{$file_folder_name}", "caching");
				} else {
					$pegasaas->utils->log("Semaphore 'html_write_cache_{$file_folder_name}': OK", "caching");
			...
			$cache_data_map["$object_id"] 	= array("id" => $object_id, "built" => time(), "css_validation_issue" => $css_validation_issue);
			if ($temp_cache) {
				$cache_data['is_temp'] = true;
	...
	*/
	...
	function clear_post_types_page_cache($post_id) {
		...
		if (array_key_exists($post_type, $pegasaas->post_type_pages) && $post_type != "page" && $post_type != "post") {
			foreach ($pegasaas->post_type_pages["$post_type"] as $object_id => $found) {
				...
				//	$pegasaas->utils->log("PegasaasCache::clear_post_types_page_cache of $post_type in $object_id", "caching");
	...
	//	exit;
		...
		/*
											...
											$pegasaas->utils->log("clear_page_cache semaphore lock on pegasaas_deferred", "caching");

				if ($output_json) {
					print json_encode(array("status" => 0, "message" => 'Error: Semaphore Lock On pegasaas_deferred_js'));
				}
			}
			$pegasaas->utils->release_semaphore("pegasaas_cache_map");
		} else {
			$pegasaas->utils->log("clear_page_cache semaphore lock on cache_map", "caching");

			if ($output_json) {
				print json_encode(array("status" => 0, "message" => 'Error: Semaphore Lock ON pegasaas_cache_map'));
			}
		}
		*/
	}
