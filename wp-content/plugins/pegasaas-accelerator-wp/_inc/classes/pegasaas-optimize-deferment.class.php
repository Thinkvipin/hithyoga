...
class PegasaasDeferment {
	...
	static function assert_global_cpcss() {
		$debug = false;
	...
	//	$buffer .= "x".PegasaasAccelerator::$settings['settings']['defer_render_blocking_js']['override'];
		...
		//	print "src = $src<br>";
			...
			// ensure that it is loaded asynchronously
			if (strstr($src, "//maps.google.com/maps/api") || strstr($src, "//maps.googleapis.com/maps/api")) {
				if (strstr($src, "async") !== true) {
					$replace = str_replace("<script ", "<script async ", $find);
					$buffer = str_replace($find, $replace, $buffer);
					$find = $replace;
				}
				continue; 
...
*/
					...
					if ($src == "") {
			 			if (strstr($find, "WebFontConfig")) {
							...
							$replace = $find;
								if (strstr($replace, "pegasaas-inline") || strstr($replace, "window.googlesitekitAdminbar")) {
									...
									continue;
								} else if (strstr($replace, "window.addEventListener")) {
									continue;
								} else if (!$have_src) {
									continue;
						...
						//if (strstr($src, "jquery.js") || strstr($src, "jquery.min.js")) {
								$jquery_signature = $replace;
								
							} else if (strstr($src, "jquery-migrate.min.js")) {
		...
		// load event handler 
		$deferral_script = "https://pegasaas.io/cache/0/d8.min.js";
