...
if (PegasaasAccelerator::$settings['status'] > 0 ) {  ?>
	...
	<path id="curve" d="M11,35 C-4.5,20.5  9.5,3.5 21,4.5 M21,4.5 C32.5,3.5 46.5,20.5 31,35"  fill="none" stroke="ff0000" stroke-width="2px" />
	  <text width="34" height="34" style='color:#fff; font-size: 1.45px' fill="#fff">
...
<div class='pgr-r'><i class='fa <?php echo $primary_change_icon_desktop; ?>'></i> <?php echo number_format($benchmark_data['desktop_score_change'], 0, '.', ','); ?></div>
			...
			<?php 
	$show_analytics_warning  = sizeof($score_data['analytics_detected']) > 0;
	...
	if ($number_of_icons == 2 && $show_analytics_warning) { 
				...
				<span class='advanced-display-settings fade'><i class="material-icons"
					...
					<input rel="fci" onchange="toggle_advanced_web_perf_display_change(this)" type="checkbox" class="material-selector" id="advanced_web_perf_fci" <?php if ($pegasaas->interface->web_perf_metrics_fci_class == "") { print "checked"; }?>><label for="advanced_web_perf_fci" class="material-selector">First CPU Idle</label><br>
