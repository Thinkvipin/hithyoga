...
if (!function_exists('yogastudio_revslider_theme_setup')) {
	...
	function yogastudio_revslider_theme_setup() {
		if (yogastudio_exists_revslider()) {
			...
			if (is_admin()) {
				add_filter( 'yogastudio_filter_importer_options',			'yogastudio_revslider_importer_set_options', 10, 2 );
				...
				add_action( 'yogastudio_action_importer_export',			'yogastudio_revslider_importer_export', 10, 1 );
				add_action( 'yogastudio_action_importer_export_fields',	'yogastudio_revslider_importer_export_fields', 10, 1 );
			}
		}
		if (is_admin()) {
...
if ( !function_exists( 'yogastudio_revslider_settings_theme_setup2' ) ) {
	...
	function yogastudio_revslider_settings_theme_setup2() {
		if (yogastudio_exists_revslider()) {
			...
			yogastudio_storage_set_array_after('options', 'slider_engine', "slider_alias", array(
				"title" => esc_html__('Revolution Slider: Select slider',  'yogastudio'),
				"desc" => wp_kses_data( __("Select slider to show (if engine=revo in the field above)", 'yogastudio') ),
				...
				"std" => "",
				"options" => yogastudio_get_options_param('list_revo_sliders'),
				"type" => "select"
				)
			);
...
if ( !function_exists( 'yogastudio_revslider_required_plugins' ) ) {
	//Handler of add_filter('yogastudio_filter_required_plugins',	'yogastudio_revslider_required_plugins');
	function yogastudio_revslider_required_plugins($list=array()) {
		if (in_array('revslider', yogastudio_storage_get('required_plugins'))) {
			$path = yogastudio_get_file_dir('plugins/install/revslider.zip');
			if (file_exists($path)) {
				$list[] = array(
					'name' 		=> esc_html__('Revolution Slider', 'yogastudio'),
					'slug' 		=> 'revslider',
					'source'	=> $path,
					'required' 	=> false
					);
...
if ( !function_exists( 'yogastudio_revslider_importer_required_plugins' ) ) {
	//Handler of add_filter( 'yogastudio_filter_importer_required_plugins',	'yogastudio_revslider_importer_required_plugins', 10, 2 );
...
if ( !function_exists( 'yogastudio_revslider_importer_set_options' ) ) {
	//Handler of add_filter( 'yogastudio_filter_importer_options',	'yogastudio_revslider_importer_set_options', 10, 2 );
	function yogastudio_revslider_importer_set_options($options=array()) {
		if (yogastudio_exists_revslider() && in_array('revslider', yogastudio_storage_get('required_plugins'))) {
...
if ( !function_exists( 'yogastudio_revslider_importer_show_params' ) ) {
	//Handler of add_action( 'yogastudio_action_importer_params',	'yogastudio_revslider_importer_show_params', 10, 1 );
	function yogastudio_revslider_importer_show_params($importer) {
		if (!empty($importer->options['files'][$importer->options['demo_type']]['file_with_revslider'])) {
			$importer->show_importer_params(array(
				'slug' => 'revslider',
				'title' => esc_html__('Import Revolution Sliders', 'yogastudio'),
				'part' => 1
				));
		}
	}
...
if ( !function_exists( 'yogastudio_revslider_importer_clear_tables' ) ) {
	//Handler of add_action( 'yogastudio_action_importer_clear_tables',	'yogastudio_revslider_importer_clear_tables', 10, 2 );
...
if ( !function_exists( 'yogastudio_revslider_importer_import' ) ) {
	//Handler of add_action( 'yogastudio_action_importer_import',	'yogastudio_revslider_importer_import', 10, 2 );
	function yogastudio_revslider_importer_import($importer, $action) {
		if ( $action == 'import_revslider' && !empty($importer->options['files'][$importer->options['demo_type']]['file_with_revslider']) ) {
			if (file_exists(WP_PLUGIN_DIR . '/revslider/revslider.php')) {
				...
				if ( ($txt = get_option('yogastudio_import_revsliders')) == '' ) {
					if ( ($txt = $importer->get_file($importer->options['files'][$importer->options['demo_type']]['file_with_revslider'])) === false)
						return;
					else
						update_option('yogastudio_import_revsliders', $txt);
				}
				$files = yogastudio_unserialize($txt);
				...
				foreach ($files as $file) {
					...
					if ( ($zip = $importer->download_file($file, round(max(0, $counter-1) / count($files) * 100))) != '') {
						...
						if ($response["success"] == false) {
							$msg = sprintf(esc_html__('Revolution Slider "%s" import error.', 'yogastudio'), $file);
							...
							if ($importer->options['debug'])
								dfl( sprintf(__('Slider "%s" imported', 'yogastudio'), basename($file)) );
						}
					}
					break;
				}
				if ($counter == count($files)) {
					update_option('yogastudio_import_revsliders', '');
				}
			} else {
				if ($importer->options['debug'])
					dfl( sprintf(__('Can not locate plugin Revolution Slider: %s', 'yogastudio'), WP_PLUGIN_DIR.'/revslider/revslider.php') );
			}
		}
	}
}
...
if ( !function_exists( 'yogastudio_revslider_importer_import_fields' ) ) {
	//Handler of add_action( 'yogastudio_action_importer_import_fields',	'yogastudio_revslider_importer_import_fields', 10, 1 );
	function yogastudio_revslider_importer_import_fields($importer) {
		$importer->show_importer_fields(array(
			'slug' => 'revslider',
			'title' => esc_html__('Revolution Slider', 'yogastudio')
			));
	}
}

// Export posts
if ( !function_exists( 'yogastudio_revslider_importer_export' ) ) {
	//Handler of add_action( 'yogastudio_action_importer_export',	'yogastudio_revslider_importer_export', 10, 1 );
	function yogastudio_revslider_importer_export($importer) {
		// Sliders list
		yogastudio_fpc(yogastudio_get_file_dir('core/core.importer/export/revslider.txt'), join("\n", array_keys(yogastudio_get_list_revo_sliders())));
	}
}

// Display exported data in the fields
if ( !function_exists( 'yogastudio_revslider_importer_export_fields' ) ) {
	//Handler of add_action( 'yogastudio_action_importer_export_fields',	'yogastudio_revslider_importer_export_fields', 10, 1 );
	function yogastudio_revslider_importer_export_fields($importer) {
		$importer->show_exporter_fields(array(
			'slug' => 'revslider',
			'title' => esc_html__('Revolution Sliders', 'yogastudio')
			));
	}
}
...
if ( !function_exists( 'yogastudio_revslider_list_sliders' ) ) {
	//Handler of add_filter( 'yogastudio_filter_list_sliders',					'yogastudio_revslider_list_sliders' );
...
if ( !function_exists( 'yogastudio_get_list_revo_sliders' ) ) {
	function yogastudio_get_list_revo_sliders($prepend_inherit=false) {
		if (($list = yogastudio_storage_get('list_revo_sliders'))=='') {
			$list = array();
			if (yogastudio_exists_revslider()) {
				global $wpdb;
			...
			$list = apply_filters('yogastudio_filter_list_revo_sliders', $list);
			if (yogastudio_get_theme_setting('use_list_cache')) yogastudio_storage_set('list_revo_sliders', $list);
		}
...
if ( !function_exists( 'yogastudio_revslider_shortcodes_params' ) ) {
	//Handler of add_filter( 'yogastudio_filter_shortcodes_params',			'yogastudio_revslider_shortcodes_params' );
...
if ( !function_exists( 'yogastudio_revslider_theme_options_params' ) ) {
	//Handler of add_filter( 'yogastudio_filter_theme_options_params',			'yogastudio_revslider_theme_options_params' );
	function yogastudio_revslider_theme_options_params($list=array()) {
		$list["list_revo_sliders"] = array('$yogastudio_get_list_revo_sliders' => '');
		return $list;
	}
}
?>