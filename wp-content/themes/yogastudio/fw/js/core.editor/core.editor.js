jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(YOGASTUDIO_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == YOGASTUDIO_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(YOGASTUDIO_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				yogastudio_message_success('', YOGASTUDIO_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(YOGASTUDIO_STORAGE['strings']['editor_caption_close']);
			} else {
				yogastudio_message_warning(rez.error, YOGASTUDIO_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		yogastudio_message_confirm(YOGASTUDIO_STORAGE['strings']['editor_delete_post'], YOGASTUDIO_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(YOGASTUDIO_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					yogastudio_message_success('', YOGASTUDIO_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = YOGASTUDIO_STORAGE['site_url'];
						}, 1000);
				} else {
					yogastudio_message_warning(rez.error, YOGASTUDIO_STORAGE['strings']['editor_delete_error']);
				}
			});
