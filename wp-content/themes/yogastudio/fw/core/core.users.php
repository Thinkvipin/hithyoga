...
if (!function_exists('yogastudio_users_theme_setup')) {
	...
	function yogastudio_users_theme_setup() {
		...
		add_action('trx_utils_filter_social_login', 'yogastudio_add_social_login');

	}
}

if (!function_exists('yogastudio_add_social_login')) {
	function yogastudio_add_social_login() {
		 return yogastudio_get_theme_option('social_login');
	}
}
	...
	function yogastudio_show_user_socials($args) {
		...
		if (count($list) > 0) {
			$output = '<div class="sc_socials sc_socials_size_'.esc_attr($args['size']).' sc_socials_shape_'.esc_attr($args['shape']).' sc_socials_type_' . esc_attr($args['style']) . '">'
							. trim(yogastudio_prepare_socials($list, $args['style']))
						. '</div>';
		...
		if (is_array($social_list) && count($social_list) > 0) {
			foreach ($social_list as $soc) {
				...
				if (!empty($sn)) {
					?>
					<tr>
						<th><label for="user_<?php echo esc_attr($sn); ?>"><?php yogastudio_show_layout(yogastudio_strtoproper($sn)); ?>:</label></th>
