...
if (!function_exists('yogastudio_post_type_theme_setup')) {
	...
	function yogastudio_post_type_theme_setup() {
		...
		if ($override_key) {
			...
			add_filter('trx_utils_filter_override_options',		'yogastudio_post_type_add_override_options');
			add_action('admin_enqueue_scripts', 'yogastudio_post_type_admin_scripts');
			// Create meta box
			yogastudio_storage_set('post_override_options', array(
				'id' => 'post-meta-box',
...
// Add meta box
if (!function_exists('yogastudio_post_type_add_override_options')) {
	function yogastudio_post_type_add_override_options($boxes = array()) {
        $boxes[] = array_merge(yogastudio_storage_get('post_override_options'), array('callback' => 'yogastudio_post_type_show_override_options'));
        return $boxes;
	}
}

// Callback function to show fields in meta box
if (!function_exists('yogastudio_post_type_show_override_options')) {
	function yogastudio_post_type_show_override_options() {
		global $post;
		...
		$mb = yogastudio_storage_get('post_override_options');
		$post_options = yogastudio_array_merge(yogastudio_storage_get('options'), $mb['fields']);

		do_action('yogastudio_action_post_before_show_override_options', $post_type, $post->ID);
		...
		do_action('yogastudio_action_post_after_show_override_options', $post_type, $post->ID);
...
if (!function_exists('yogastudio_post_type_save_options')) {
	//Handler of add_action('save_post', 'yogastudio_post_type_save_options');
	function yogastudio_post_type_save_options($post_id) {

		// verify nonce
		if ( !wp_verify_nonce( yogastudio_get_value_gp('override_options_post_nonce'), admin_url() ) )
			return $post_id;
		...
		$post_options = array_merge(yogastudio_storage_get('options'), yogastudio_storage_get_array('post_override_options', 'fields'));
