	...
	function yogastudio_options_theme_setup() {
		...
		if ( is_admin() ) {
			...
			if ( yogastudio_options_is_used() ) {
				...
				//Handler of add_filter("yogastudio_filter_save_options",		'yogastudio_options_save_stylesheet', 10, 3);
				...
				yogastudio_storage_set('to_data', null);
				yogastudio_storage_set('to_delimiter', ',');
				yogastudio_storage_set('to_colorpicker', 'tiny');			// wp - WP colorpicker, custom - internal theme colorpicker, tiny - external script
			}
		}
...
if ( !function_exists( 'yogastudio_options_admin_menu_item' ) ) {
	//Handler of add_action('admin_menu', 'yogastudio_options_admin_menu_item');
	...
	function yogastudio_options_is_used() {
		...
		if (is_admin()) {
			if (isset($_REQUEST['action']) && ($_REQUEST['action']=='yogastudio_options_save' || $_REQUEST['action']=='yogastudio_options_import'))		// AJAX: Save or Import Theme Options
				$used = true;
			else if (yogastudio_strpos(add_query_arg(array()), 'yogastudio_options')!==false)															// Edit Theme Options
				$used = true;
			else if (yogastudio_strpos(add_query_arg(array()), 'post-new.php')!==false || yogastudio_strpos(add_query_arg(array()), 'post.php')!==false) {	// Create or Edit Post (page, product, ...)
				$post_type = yogastudio_admin_get_current_post_type();
				if (empty($post_type)) $post_type = 'post';
				$used = yogastudio_get_override_key($post_type, 'post_type')!='';
			} else if (yogastudio_strpos(add_query_arg(array()), 'edit-tags.php')!==false) {															// Edit Taxonomy
					...
					foreach ($inheritance as $k=>$v) {
						if (!empty($v['taxonomy']) && is_array($v['taxonomy'])) {
							foreach ($v['taxonomy'] as $tax) {
								if ( yogastudio_strpos(add_query_arg(array()), 'taxonomy='.($tax))!==false && in_array($post_type, $v['post_type']) ) {
									$used = true;
									break;
								}
							}
...
if ( !function_exists( 'yogastudio_load_main_options' ) ) {
	function yogastudio_load_main_options() {
		$values = get_option(yogastudio_storage_get('options_prefix') . '_options', array());
		$options = yogastudio_storage_get('options');
		if (is_array($options) && count($options) > 0) {
			...
			yogastudio_storage_set('options', $options);
		}
...
if ( !function_exists( 'yogastudio_load_custom_options' ) ) {
	function yogastudio_load_custom_options() {
		...
		yogastudio_storage_set('custom_options', array());
		yogastudio_storage_set('post_options', array());
		yogastudio_storage_set('taxonomy_options', array());
		yogastudio_storage_set('template_options', array());
		yogastudio_storage_set('theme_options_loaded', false);
		
		if ( is_admin() ) {
			yogastudio_storage_set('theme_options_loaded', true);
			return;
		}
		...
		if ( !empty($slug) ) {
			...
			if (empty($inheritance['use_options_page']) || $inheritance['use_options_page'])
				$tmp = get_option(yogastudio_storage_get('options_prefix') . '_options_template_'.trim($slug));
			...
			if ($tmp===false && !empty($inheritance['override'])) {
				...
				if ( !empty($slug) ) $tmp = get_option(yogastudio_storage_get('options_prefix') . '_options_template_'.trim($slug));
			}
			yogastudio_storage_set('template_options', $tmp===false ? array() : $tmp);
		}
			...
			if (!empty($inheritance['taxonomy']) && is_array($inheritance['taxonomy'])) {
				foreach ($inheritance['taxonomy'] as $tax) {
					...
					if ($tax == 'category' && is_category()) {		// Current page is category's archive (Categories need specific check)
						...
						yogastudio_storage_set('taxonomy_options', yogastudio_taxonomy_get_inherited_properties('category', $tax_id));
						break;
					} else if ($tax == 'post_tag' && is_tag()) {	// Current page is tag's archive (Tags need specific check)
						$tax_id = get_query_var( $tax_query );
						yogastudio_storage_set('taxonomy_options', yogastudio_taxonomy_get_inherited_properties('post_tag', $tax_id));
						break;
					} else if (is_tax($tax)) {						// Current page is custom taxonomy archive (All rest taxonomies check)
						$tax_id = get_query_var( $tax_query );
						yogastudio_storage_set('taxonomy_options', yogastudio_taxonomy_get_inherited_properties($tax, $tax_id));
						break;
					}
				}
			}
			// Load post options
			if ( is_singular() && (!yogastudio_storage_empty('page_template') || !yogastudio_storage_get('blog_streampage')) ) {
				$post_id = get_the_ID();
				if ( $post_id == 0 && !empty($wp_query->queried_object_id) ) $post_id = $wp_query->queried_object_id;
				yogastudio_storage_set('post_options', get_post_meta($post_id, 'yogastudio_post_options', true));
				if ( !empty($inheritance['post_type']) && !empty($inheritance['taxonomy'])
					...
					if (!empty($tax_list)) {
						$tmp = yogastudio_storage_get('taxonomy_options');
						foreach($tax_list as $tax_options) {
							if (!empty($tax_options) && is_array($tax_options)) {
								foreach($tax_options as $tk=>$tv) {
									if ( !isset($tmp[$tk]) || yogastudio_is_inherit_option($tmp[$tk]) ) {
										$tmp[$tk] = $tv;
									}
								}
							}
						}
						yogastudio_storage_set('taxonomy_options', $tmp);
					}
				}
			}
		}
		...
		$layout_name = yogastudio_get_custom_option(is_singular() && !yogastudio_storage_get('blog_streampage') ? 'single_style' : 'blog_style');
		if (!yogastudio_storage_empty('registered_templates', $layout_name, 'theme_options')) {
			yogastudio_storage_set('template_options', array_merge(yogastudio_storage_get('template_options'), yogastudio_storage_get_array('registered_templates', $layout_name, 'theme_options')));
		}
...
if ( !function_exists( 'yogastudio_get_theme_setting' ) ) {
	function yogastudio_get_theme_setting($option_name, $default='') {
		return yogastudio_storage_get_array('settings', $option_name, $default);
	}
}
...
if ( !function_exists( 'yogastudio_set_theme_setting' ) ) {
	function yogastudio_set_theme_setting($option_name, $value) {
		yogastudio_storage_set_array('settings', $option_name, $value);
	}
}
...
if ( !function_exists( 'yogastudio_get_theme_option' ) ) {
	function yogastudio_get_theme_option($option_name, $default = false, $options = null) {
		...
		} else if (yogastudio_storage_isset('options', $option_name, 'val')) {
			$val = yogastudio_storage_get_array('options', $option_name, 'val');
		} else {
			if ($last_options===false) $last_options = get_option(yogastudio_storage_get('options_prefix') . '_options', array());
			if (isset($last_options[$option_name])) {
				$val = $last_options[$option_name];
			} else if (yogastudio_storage_isset('options', $option_name, 'std')) {
				$val = yogastudio_storage_get_array('options', $option_name, 'std');
			}
		}
		if ($val === '') {	//false) {
...
if ( !function_exists( 'yogastudio_get_custom_option' ) ) {
	function yogastudio_get_custom_option($name, $defa=null, $post_id=0, $post_type='post', $tax_id=0, $tax_type='category') {
		if (isset($_GET[$name]))
			...
			if (!yogastudio_storage_empty('theme_options_loaded') && yogastudio_storage_isset('custom_options', $hash_name)) {
				$rez = yogastudio_storage_get_array('custom_options', $hash_name);
			} else {
				if ($tax_id > 0) {
					...
					if (yogastudio_storage_isset('template_options', $name)) {
						 $tmp = yogastudio_storage_get_array('template_options', $name);
						 if (!yogastudio_is_inherit_option($tmp)) $rez = is_array($tmp) ? $tmp[0] : $tmp;
					}
					if (yogastudio_storage_isset('taxonomy_options', $name)) {
						 $tmp = yogastudio_storage_get_array('taxonomy_options', $name);
						 if (!yogastudio_is_inherit_option($tmp)) $rez = $tmp;
					}
					if (yogastudio_storage_isset('post_options', $name)) {
						 $tmp = yogastudio_storage_get_array('post_options', $name);
						 if (!yogastudio_is_inherit_option($tmp)) $rez = is_array($tmp) ? $tmp[0] : $tmp;
					}
				}
				$rez = apply_filters('yogastudio_filter_get_custom_option', $rez, $name);
				if (!yogastudio_storage_empty('theme_options_loaded')) yogastudio_storage_set_array('custom_options', $hash_name, $rez);
			}
		}
		return $rez;
	}
...
if ( !function_exists( 'yogastudio_get_options_param' ) ) {
	function yogastudio_get_options_param($prm) {
		return yogastudio_storage_get_array('options_params', $prm);
	}
}

// Set options_param value
if ( !function_exists( 'yogastudio_set_options_param' ) ) {
	function yogastudio_set_options_param($prm, $val) {
		yogastudio_storage_set_array('options_params', $prm, $val);
	}
}
...
if ( !function_exists( 'yogastudio_options_load_scripts' ) ) {
	function yogastudio_options_load_scripts() {
		...
		wp_enqueue_style( 'yogastudio-fontello-admin-style',	yogastudio_get_file_url('css/fontello-admin/css/fontello-admin.css'), array(), null);
		wp_enqueue_style( 'yogastudio-fontello-style', 			yogastudio_get_file_url('css/fontello/css/fontello.css'), array(), null);
		wp_enqueue_style( 'yogastudio-fontello-animation-style',yogastudio_get_file_url('css/fontello-admin/css/animation.css'), array(), null);
		// Yogastudio options styles
		wp_enqueue_style('yogastudio-options-style',			yogastudio_get_file_url('core/core.options/css/core.options.css'), array(), null);
		wp_enqueue_style('yogastudio-options-datepicker-style',	yogastudio_get_file_url('core/core.options/css/core.options-datepicker.css'), array(), null);
		...
		// Color Picker
			...
			wp_enqueue_script('yogastudio-colors-script',		yogastudio_get_file_url('js/colorpicker/colors.js'), array('jquery'), null, true );
			//wp_enqueue_style( 'yogastudio-colorpicker-style',	yogastudio_get_file_url('js/colorpicker/jqColorPicker.css'), array(), null);
			wp_enqueue_script('yogastudio-colorpicker-script',	yogastudio_get_file_url('js/colorpicker/jqColorPicker.js'), array('jquery'), null, true );

		// Input masks for text fields
		wp_enqueue_script( 'jquery-input-mask',				yogastudio_get_file_url('core/core.options/js/jquery.maskedinput.1.3.1.min.js'), array('jquery'), null, true );
		// Yogastudio core scripts
		wp_enqueue_script( 'yogastudio-core-utils-script',		yogastudio_get_file_url('js/core.utils.js'), array(), null, true );
		// Yogastudio options scripts
		wp_enqueue_script( 'yogastudio-options-script',			yogastudio_get_file_url('core/core.options/js/core.options.js'), array('jquery', 'jquery-ui-core', 'jquery-ui-tabs', 'jquery-ui-accordion', 'jquery-ui-sortable', 'jquery-ui-draggable', 'jquery-ui-datepicker'), null, true );
		wp_enqueue_script( 'yogastudio-options-custom-script',	yogastudio_get_file_url('core/core.options/js/core.options-custom.js'), array('yogastudio-options-script'), null, true );
...
if ( !function_exists( 'yogastudio_options_prepare_scripts' ) ) {
	function yogastudio_options_prepare_scripts($override='') {
        ...
        yogastudio_storage_set_array('js_vars', 'to_options', yogastudio_storage_get('to_data') );
        yogastudio_storage_set_array('js_vars', 'to_delimiter', esc_attr(yogastudio_storage_get('to_delimiter')));
        yogastudio_storage_set_array('js_vars', 'to_slug', esc_attr(yogastudio_storage_get_array('to_flags', 'slug')));
        yogastudio_storage_set_array('js_vars', 'to_popup', esc_attr(yogastudio_get_theme_option('popup_engine')));
        yogastudio_storage_set_array('js_vars', 'to_override', esc_attr($override));
        $keys = array();
        if (($export_opts = get_option(yogastudio_storage_get('options_prefix') . '_options_export_'.($override), false)) !== false) {
            $keys = join(',', array_keys($export_opts));
        }
        yogastudio_storage_set_array('js_vars', 'to_export_list', $keys);
        yogastudio_storage_merge_array('js_vars', 'to_strings', array(
            'del_item_error' => esc_html__("You can't delete last item! To disable it - just clear value in field.", 'yogastudio'),
            'del_item' => esc_html__("Delete item error!", 'yogastudio'),
            'recompile_styles' => esc_html__("When saving color schemes and font settings, recompilation of .less files occurs. It may take from 5 to 15 secs dependning on your server's speed and size of .less files.", 'yogastudio'),
            'wait' => esc_html__("Please wait a few seconds!", 'yogastudio'),
            'reload_page' => esc_html__("After 3 seconds this page will be reloaded.", 'yogastudio'),
            'save_options' => esc_html__("Options saved!", 'yogastudio'),
            'reset_options' => esc_html__("Options reset!", 'yogastudio'),
            'reset_options_confirm' => esc_html__("Do you really want reset all options to default values?", 'yogastudio'),
            'reset_options_complete' => esc_html__("Settings are reset to their default values.", 'yogastudio'),
            'export_options_header' => esc_html__("Export options", 'yogastudio'),
            'export_options_error' => esc_html__("Name for options set is not selected! Export cancelled.", 'yogastudio'),
            'export_options_label' => esc_html__("Name for the options set:", 'yogastudio'),
            'export_options_label2' => esc_html__("or select one of exists set (for replace):", 'yogastudio'),
            'export_options_select' => esc_html__("Select set for replace ...", 'yogastudio'),
            'export_empty' => esc_html__("No exported sets for import!", 'yogastudio'),
            'export_options' => esc_html__("Options exported!", 'yogastudio'),
            'export_link' => esc_html__("If need, you can download the configuration file from the following link: %s", 'yogastudio'),
            'export_download' => esc_html__("Download theme options settings", 'yogastudio'),
            'import_options_label' => esc_html__("or put here previously exported data:", 'yogastudio'),
            'import_options_label2' => esc_html__("or select file with saved settings:", 'yogastudio'),
            'import_options_header' => esc_html__("Import options", 'yogastudio'),
            'import_options_error' => esc_html__("You need select the name for options set or paste import data! Import cancelled.", 'yogastudio'),
            'import_options_failed' => esc_html__("Error while import options! Import cancelled.", 'yogastudio'),
            'import_options_broken' => esc_html__("Attention! Some options are not imported:", 'yogastudio'),
            'import_options' => esc_html__("Options imported!", 'yogastudio'),
            'import_dummy_confirm' => esc_html__("Attention! During the import process, all existing data will be replaced with new.", 'yogastudio'),
            'clear_cache' => esc_html__("Cache cleared successfull!", 'yogastudio'),
            'clear_cache_header' => esc_html__("Clear cache", 'yogastudio'),
        ));
	}
}
...
if ( !function_exists( 'yogastudio_options_page' ) ) {
	function yogastudio_options_page() {
		...
		$custom_options = empty($override) ? false : get_option(yogastudio_storage_get('options_prefix') . '_options'.(!empty($slug) ? '_template_'.trim($slug) : ''));
		...
		yogastudio_options_page_start(array(
			...
			'description' => empty($slug) 
								? (empty($override) 
									? wp_kses_data( __("Global settings affect the entire website's display. They can be overriden when editing settings for a certain post type (if select it in the popup menu above) or when editing category or single page/post (affect only on this category/page/post)", 'yogastudio') )
									: '') 
								: wp_kses_data( __('Settings template for a certain post type: affects the display of just one specific post type. They can be overriden when editing categories and/or posts of a certain type', 'yogastudio') ),
			'subpage' => $mode,
			'slug' => $slug,
			'override' => $override
		));

		$to_data = yogastudio_storage_get('to_data');
...
if ( !function_exists( 'yogastudio_options_page_start' ) ) {
	function yogastudio_options_page_start($args = array()) {
		...
		yogastudio_storage_set('to_flags', $to_flags);
		yogastudio_storage_set('to_data', empty($args['data']) ? yogastudio_storage_get('options') : $args['data']);
		// Load required styles and scripts for Options Page
			...
			<?php }	?>
				<div class="yogastudio_options_header">
					<div id="yogastudio_options_logo" class="yogastudio_options_logo">
						...
						<h2><?php yogastudio_show_layout($to_flags['title']); ?></h2>
					</div>
					...
					<div id="yogastudio_options_title" class="yogastudio_options_title">
						...
						?>
						<p><?php yogastudio_show_layout($to_flags['description']); ?></p>
					</div>
				</div>
...
if ( !function_exists( 'yogastudio_options_page_stop' ) ) {
	function yogastudio_options_page_stop() {
		yogastudio_show_layout(yogastudio_options_close_nested_groups('', true));
				?>
				</div> <!-- .yogastudio_options_body -->
				<?php
		if (yogastudio_storage_get_array('to_flags', 'create_form')) {
			?>
			</form>
			<?php
		}
...
if ( !function_exists( 'yogastudio_options_close_nested_groups' ) ) {
	function yogastudio_options_close_nested_groups($type='', $end=false) {
		$output = '';
		$nesting = yogastudio_storage_get_array('to_flags', 'nesting');
		if ($nesting) {
			...
			yogastudio_storage_set_array('to_flags', 'nesting', $nesting);
		}
		return $output;
	}
}
	...
	function yogastudio_options_collect_tabs($type, $id) {
		...
		if (is_array($to_data) && count($to_data) > 0) {
			foreach ($to_data as $field_id=>$field) {
				if (!yogastudio_storage_empty('to_flags', 'override') && (empty($field['override']) || !in_array(yogastudio_storage_get_array('to_flags', 'override'), explode(',', $field['override'])))) continue;
...
if ( !function_exists( 'yogastudio_options_menu_list' ) ) {
	function yogastudio_options_menu_list($field, $clone_val) {
		...
		$to_delimiter = yogastudio_storage_get('to_delimiter');
...
if ( !function_exists( 'yogastudio_options_show_field' ) ) {
	function yogastudio_options_show_field($id, $field, $value=null) {
		...
		if (!empty($field['subset'])) {
			$sbs = yogastudio_get_theme_option($field['subset'], '', yogastudio_storage_get('to_data'));
		...
		if ($field['type']=='color' && (yogastudio_storage_get('to_colorpicker')=='tiny' || (isset($field['style']) && $field['style']!='wp'))) {
			if (!isset($field['after'])) $field['after'] = array();
			$field['after'] = array_merge(array(
					'icon'=>'iconadmin-cancel',
					'action'=>'color_reset'
		...
		if (yogastudio_options_is_group($field['type'])) {					// Close nested containers
			if (empty($field['start']) && (!in_array($field['type'], array('group', 'toggle')) || !empty($field['end']))) {
				yogastudio_show_layout(yogastudio_options_close_nested_groups($field['type'], !empty($field['end'])));
				if (!empty($field['end'])) {
					return;
				}
			}
			...
			if ($field['type'] != 'hidden') {
				...
				if ( !in_array($field['type'], array('divider'))) {
					echo '<label class="yogastudio_options_field_label'
						. (!yogastudio_storage_empty('to_flags', 'add_inherit') && isset($field['std']) ? ' yogastudio_options_field_label_inherit' : '')
						. '"'
						. (!empty($field['title']) ? ' for="'.esc_attr($id).'"' : '')
						. '>' 
						. ($field['title']) 
						. (!empty($field['info']) && ($fdir=yogastudio_get_file_url('images/to_info/'.sanitize_file_name($id).'.jpg'))!=''
							? '<a href="'.esc_url($fdir).'" data-rel="popup" target="_blank" class="yogastudio_options_field_label_info iconadmin-info-circled" title="'.esc_html__('More info', 'yogastudio').'"></a>'
							: '')
						. (!yogastudio_storage_empty('to_flags', 'add_inherit') && isset($field['std'])
		...
		foreach ($field['val'] as $clone_num => $clone_val) {
			...
			case 'group':
				...
				yogastudio_storage_push_array('to_flags', 'nesting', 'group');
			break;
		
			case 'toggle':
				yogastudio_storage_push_array('to_flags', 'nesting', 'toggle');
			...
			case 'accordion':
				yogastudio_storage_push_array('to_flags', 'nesting', 'accordion');
			...
			case 'tab':
				yogastudio_storage_push_array('to_flags', 'nesting', 'tab');
				if (!empty($field['start']))
			...
			case 'partition':
				yogastudio_storage_push_array('to_flags', 'nesting', 'partition');
				if (!empty($field['start']))
			...
			case 'media':
				...
				if (!empty($clone_val) && !yogastudio_is_inherit_option($clone_val)) {
					...
					echo '<a class="yogastudio_options_image_preview" data-rel="popup" target="_blank" href="'.esc_url($clone_val).'">'
							. (!empty($ext) && yogastudio_strpos('jpg,png,gif', $ext)!==false
									? '<img src="'.esc_url($clone_val).'" alt="'.esc_attr__('img', 'yogastudio').'" />'
			...
			case 'range':
				...
				if (yogastudio_strpos($clone_val, $to_delimiter)===false)
					$clone_val = max($field['min'], intval($clone_val));
				if (yogastudio_strpos($field['std'], $to_delimiter)!==false && yogastudio_strpos($clone_val, $to_delimiter)===false)
				...
				if (is_array($field['options']) && count($field['options']) > 0) {
					foreach ($field['options'] as $key => $title) { 
						echo '<span class="yogastudio_options_listitem'
							. (yogastudio_strpos(($to_delimiter).($clone_val).($to_delimiter), ($to_delimiter).($key).($to_delimiter))!==false ? ' yogastudio_options_state_checked' : '') . '"'
							. ' data-value="'.esc_attr($key).'"'
							. '>'
							. esc_attr($title)
							. '</span>';
			...
			case "color":
				$cp_style = isset($field['style']) ? $field['style'] : yogastudio_storage_get('to_colorpicker');
				...
				if ($cp_style=='custom')
					...
					yogastudio_show_layout($after);
			break;   
	
			default:
				if (function_exists('yogastudio_show_custom_field')) {
					yogastudio_show_layout(yogastudio_show_custom_field($id, $field, $clone_val));
				}
			} 
		...
		if (!yogastudio_options_is_group($field['type']) && $field['type'] != 'hidden') {
			...
			if (!yogastudio_storage_empty('to_flags', 'add_inherit') && isset($field['std']))
...
if ( !function_exists( 'yogastudio_options_save' ) ) {
	//Handler of add_action('wp_ajax_yogastudio_options_save', 'yogastudio_options_save');
	//Handler of add_action('wp_ajax_nopriv_yogastudio_options_save', 'yogastudio_options_save');
	function yogastudio_options_save() {
		...
		if ($mode == 'save') {
			...
			parse_str($_POST['data'], $post_data);
			if (!yogastudio_storage_empty('post_override_options', 'fields')) {
				$options = yogastudio_array_merge(yogastudio_storage_get('options'), yogastudio_storage_get_array('post_override_options', 'fields'));
			}
		} else
			$post_data = array();
		...
		if ($mode=='export') {
			...
			$key = $name=='' ? $name2 : $name;
			$export = get_option(yogastudio_storage_get('options_prefix') . '_options_export_'.($override), array());
			$export[$key] = $custom_options;
			if ($name!='' && $name2!='') unset($export[$name2]);
			update_option(yogastudio_storage_get('options_prefix') . '_options_export_'.($override), $export);
			...
			update_option(yogastudio_storage_get('options_prefix') . '_options'.(!empty($slug) ? '_template_'.trim($slug) : ''), apply_filters('yogastudio_filter_save_options', $custom_options, $override, $slug));
			if ($override=='general') {
...
if ( !function_exists( 'yogastudio_options_import' ) ) {
	//Handler of add_action('wp_ajax_yogastudio_options_import', 'yogastudio_options_import');
	//Handler of add_action('wp_ajax_nopriv_yogastudio_options_import', 'yogastudio_options_import');
	function yogastudio_options_import() {
		...
		if (!empty($text)) {
			...
			$import = get_option(yogastudio_storage_get('options_prefix') . '_options_export_'.($override), array());
...
require_once YOGASTUDIO_THEME_PATH . 'includes/theme.options.php';

// Load inheritance system
require_once YOGASTUDIO_FW_PATH . 'core/core.options/core.options-inheritance.php';

// Load custom fields
if (is_admin()) {
	require_once YOGASTUDIO_FW_PATH . 'core/core.options/core.options-custom.php';
}
?>