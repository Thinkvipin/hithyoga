...
if ( !function_exists( 'yogastudio_options_custom_load_scripts' ) ) {
	//Handler of add_action("admin_enqueue_scripts", 'yogastudio_options_custom_load_scripts');
	function yogastudio_options_custom_load_scripts() {
		wp_enqueue_script( 'yogastudio-options-custom-script',	yogastudio_get_file_url('core/core.options/js/core.options-custom.js'), array(), null, true );	
	}
}
...
if ( !function_exists( 'yogastudio_show_custom_field' ) ) {
	function yogastudio_show_custom_field($id, $field, $value) {
		...
		switch ($field['type']) {
			...
			case 'mediamanager':
				wp_enqueue_media( );
				$output .= '<a id="'.esc_attr($id).'" class="button mediamanager yogastudio_media_selector"
