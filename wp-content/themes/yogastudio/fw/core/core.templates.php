...
if (!function_exists('yogastudio_add_template')) {
	function yogastudio_add_template($tpl) {
		...
		yogastudio_storage_set_array('registered_templates', $tpl['layout'], $tpl);
...
if (!function_exists('yogastudio_get_template_name')) {
	function yogastudio_get_template_name($layout_name) {
		$tpl = yogastudio_storage_get_array('registered_templates', $layout_name);
		return $tpl['template'];
	}
}
...
if (!function_exists('yogastudio_get_template_function_name')) {
	function yogastudio_get_template_function_name($layout_name) {
		$tpl = yogastudio_storage_get_array('registered_templates', $layout_name);
		return 'yogastudio_template_'.str_replace(array('-', '.'), '_', $tpl['template']).'_output';
	}
}

// Set template arguments
if (!function_exists('yogastudio_template_set_args')) {
	function yogastudio_template_set_args($tpl, $args) {
		yogastudio_storage_push_array('call_args', $tpl, $args);
	}
}


// Get template arguments
if (!function_exists('yogastudio_template_get_args')) {
	function yogastudio_template_get_args($tpl) {
		return yogastudio_storage_pop_array('call_args', $tpl, array());
	}
}


// Look for last template arguments (without removing it from storage)
if (!function_exists('yogastudio_template_last_args')) {
	function yogastudio_template_last_args($tpl) {
		$args = yogastudio_storage_get_array('call_args', $tpl, array());
...
if (!function_exists('yogastudio_add_thumb_sizes')) {
	function yogastudio_add_thumb_sizes($sizes) {
		...
		if (yogastudio_storage_get_array('thumb_sizes', $thumb_slug)=='') {
			yogastudio_storage_set_array('thumb_sizes', $thumb_slug, $sizes);
			// Register WP thumb size
			if (yogastudio_get_theme_setting('add_image_size') && (!isset($sizes['add_image_size']) || $sizes['add_image_size'])) {
...
if (!function_exists('yogastudio_get_thumb_sizes')) {
	function yogastudio_get_thumb_sizes($opt) {
		...
		$tpl = yogastudio_storage_get_array('registered_templates', $opt['layout']);
		$thumb_slug = !empty($opt['thumb_slug']) 
						? $opt['thumb_slug'] 
						: (empty($tpl['thumb_slug'])
							? (empty($tpl['thumb_title'])
								? '' 
								: yogastudio_get_slug($tpl['thumb_title'])
								) 
							: yogastudio_get_slug($tpl['thumb_slug'])
							);
		$thumb_size = yogastudio_storage_get_array('thumb_sizes', $thumb_slug);
...
if (!function_exists('yogastudio_show_thumb_sizes')) {
	function yogastudio_show_thumb_sizes( $sizes ) {
		$thumb_sizes = yogastudio_storage_get('thumb_sizes');
