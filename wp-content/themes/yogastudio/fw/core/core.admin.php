...
if (is_admin()) {
	...
	if ( !function_exists( 'yogastudio_admin_theme_setup' ) ) {
		...
		function yogastudio_admin_theme_setup() {
			if ( is_admin() ) {
				add_action("admin_footer",			'yogastudio_admin_prepare_scripts', 9);
	...
	if ( !function_exists( 'yogastudio_admin_load_scripts' ) ) {
		//Handler of add_action("admin_enqueue_scripts", 'yogastudio_admin_load_scripts');
		function yogastudio_admin_load_scripts() {
            if (yogastudio_get_theme_option('debug_mode')=='yes') {
			    wp_enqueue_script( 'yogastudio-debug-script', yogastudio_get_file_url('js/core.debug.js'), array('jquery'), null, true );
            }
				wp_enqueue_style( 'yogastudio-admin-style', yogastudio_get_file_url('css/core.admin.css'), array(), null );
				wp_enqueue_script( 'yogastudio-admin-script', yogastudio_get_file_url('js/core.admin.js'), array('jquery'), null, true );
			if (yogastudio_strpos(add_query_arg(array()), 'widgets.php')!==false) {
				wp_enqueue_style( 'yogastudio-fontello-style', yogastudio_get_file_url('css/fontello-admin/css/fontello-admin.css'), array(), null );
				wp_enqueue_style( 'yogastudio-animations-style', yogastudio_get_file_url('css/fontello-admin/css/animation.css'), array(), null );
			}
		}
	}
	...
	if ( !function_exists( 'yogastudio_admin_prepare_scripts' ) ) {
		//Handler of add_action("admin_head", 'yogastudio_admin_prepare_scripts');
		function yogastudio_admin_prepare_scripts() {
            ...
            wp_localize_script('yogastudio-admin-script', 'YOGASTUDIO_STORAGE', apply_filters('yogastudio_action_add_scripts_inline', $vars));
	...
	require_once YOGASTUDIO_FW_PATH . 'lib/tgm/class-tgm-plugin-activation.php';
}
