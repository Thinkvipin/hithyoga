	...
	jQuery('#partition_schemes_content .yogastudio_options_tab > ul').sortable({
		update: function(event, ui) {
			...
			ui.item.parents('.yogastudio_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? YOGASTUDIO_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
...
function yogastudio_options_action_scheme_delete(obj) {
	...
	var scheme = tab_id.substring(4, tab_id.length-8);
	yogastudio_message_confirm(YOGASTUDIO_STORAGE['to_strings']['scheme_delete_confirm'], YOGASTUDIO_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
		...
		setTimeout(function(){
			yogastudio_message_info(YOGASTUDIO_STORAGE['to_strings']['recompile_styles'], YOGASTUDIO_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(YOGASTUDIO_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				yogastudio_message_success(YOGASTUDIO_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+YOGASTUDIO_STORAGE['to_strings']['reload_page'], YOGASTUDIO_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				yogastudio_message_warning(YOGASTUDIO_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, YOGASTUDIO_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function yogastudio_options_action_scheme_copy(obj) {
	...
	var tab_id = button.parents('.yogastudio_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	yogastudio_message_confirm(YOGASTUDIO_STORAGE['to_strings']['scheme_copy_confirm'], YOGASTUDIO_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
		...
		jQuery.post(YOGASTUDIO_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				yogastudio_message_success(YOGASTUDIO_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+YOGASTUDIO_STORAGE['to_strings']['reload_page'], YOGASTUDIO_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				yogastudio_message_warning(YOGASTUDIO_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, YOGASTUDIO_STORAGE['to_strings']['scheme_copy']);
			}
		});
