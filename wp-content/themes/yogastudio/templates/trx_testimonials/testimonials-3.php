...
if ( !function_exists( 'yogastudio_template_testimonials_3_theme_setup' ) ) {
	add_action( 'yogastudio_action_before_init_theme', 'yogastudio_template_testimonials_3_theme_setup', 1 );
	function yogastudio_template_testimonials_3_theme_setup() {
		yogastudio_add_template(array(
			...
			'title'  => esc_html__('Testimonials /Style 3/', 'yogastudio'),
...
// Template output
if ( !function_exists( 'yogastudio_template_testimonials_3_output' ) ) {
	function yogastudio_template_testimonials_3_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> class="sc_testimonial_item<?php echo (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo !empty($post_options['tag_css']) ? ' style="'.esc_attr($post_options['tag_css']).'"' : '';?>>
				<div class="sc_testimonial_content"><?php yogastudio_show_layout($post_data['post_content']); ?>
