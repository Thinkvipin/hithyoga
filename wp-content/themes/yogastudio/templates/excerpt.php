...
if ( !function_exists( 'yogastudio_template_excerpt_output' ) ) {
	function yogastudio_template_excerpt_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] == 'center' && !empty($post_data['post_title'])) {
			?><h3 class="post_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php yogastudio_show_layout($post_data['post_title']); ?></a></h3><?php
		}
		...
		<div class="post_content clearfix">
			<?php
			if ($show_title && $post_options['location'] != 'center' && !empty($post_data['post_title'])) {
				?><h3 class="post_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php yogastudio_show_layout($post_data['post_title']); ?></a></h3><?php
			}
			...
			<div class="post_descr">
				...
				if (!yogastudio_param_is_off($post_options['readmore']) && !in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status'))) {
                    if(function_exists('yogastudio_sc_button')) yogastudio_show_layout(yogastudio_sc_button(array('link'=>$post_data['post_link']), $post_options['readmore']));
				}
				?>
			</div>
