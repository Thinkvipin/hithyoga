			...
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				<div class="sc_services_item_content">
					...
					if (isset($post_data['post_date']) && !empty($post_data['post_date'])) {
						echo '<h4 class="title_date">' . esc_attr($post_data['post_date']) .'</h4>';
					}
					if ($show_title) {
						if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
							?><h4 class="sc_services_item_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php yogastudio_show_layout((!empty($post_data['post_title']) ? $post_data['post_title'] : '')); ?></a></h4><?php
						} else {
					...
					<div class="sc_services_item_description">
						<?php
							if (!empty($post_data['post_link']) && !yogastudio_param_is_off($post_options['readmore'])) {
								?><a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_services_item_readmore"><?php yogastudio_show_layout($post_options['readmore']); ?><span class="icon-right"></span></a><?php
							}
						?>
					</div>
				</div>
