...
if ( !function_exists( 'yogastudio_template_services_1_theme_setup' ) ) {
	add_action( 'yogastudio_action_before_init_theme', 'yogastudio_template_services_1_theme_setup', 1 );
	function yogastudio_template_services_1_theme_setup() {
		yogastudio_add_template(array(
			'layout' => 'services-1',
			'template' => 'services-1',
			'mode'   => 'services',
			'title'  => esc_html__('Services /Style 1/', 'yogastudio'),
...
// Template output
if ( !function_exists( 'yogastudio_template_services_1_output' ) ) {
	function yogastudio_template_services_1_output($post_options, $post_data) {
		$show_title = true;
		...
		if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
			...
			if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
				?><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php yogastudio_show_layout($html); ?></a><?php
			} else
			yogastudio_show_layout($html);
		} else {
			?>
		...
		<div class="sc_services_item_content">
			...
			if ($show_title) {
				if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
					...
					<?php 
					yogastudio_show_layout((!empty($post_data['post_title']) ? $post_data['post_title'] : ''));
					?>
				</a></h4><?php
			} else {
	...
	<div class="sc_services_item_description">
		...
		if ($post_data['post_protected']) {
			...
			if (!empty($post_data['post_link']) && !yogastudio_param_is_off($post_options['readmore'])) {
				?><a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_services_item_readmore"><?php yogastudio_show_layout($post_options['readmore']); ?><span class="icon-play-circled"></span></a><?php
			}
		}
		?>
	</div>
