...
if ( !function_exists( 'yogastudio_template_list_theme_setup' ) ) {
	add_action( 'yogastudio_action_before_init_theme', 'yogastudio_template_list_theme_setup', 1 );
	function yogastudio_template_list_theme_setup() {
		yogastudio_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'yogastudio')
			));
	}
}

// Template output
if ( !function_exists( 'yogastudio_template_list_output' ) ) {
	function yogastudio_template_list_output($post_options, $post_data) {
