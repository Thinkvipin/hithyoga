<?php
// Get template args
extract(yogastudio_template_get_args('prev-next-block'));
...
if (!$post_data['post_protected']) {
	...
	if (!empty($post_options['dedicated'])) {
		...
		$page_style = yogastudio_get_custom_option('single_style');
		?>
		<div class="post_thumb post_nav"<?php echo 'single-portfolio-fullscreen'==$page_style ? ' style="background-image:url('.esc_url($post_data['post_attachment']).');"' : ''; ?>>
			<?php 
			if ($page_style!='single-portfolio-fullscreen') yogastudio_show_layout($post_data['post_thumb']);
			$cur = get_post();
			$args = array(
			...
			$args = yogastudio_query_add_posts_and_cats($args, '', $post_data['post_type'], 
			...
			if ( $prev ) {
				...
				$desc = yogastudio_strshort($prev->post_title, 30);
				?>
				<a class="post_nav_item post_nav_prev" href="<?php echo esc_url($link); ?>">
					<span class="post_nav_info">
						...
						<span class="post_nav_info_description"><?php yogastudio_show_layout($desc); ?></span>
					</span>
				</a>
				<?php
			}
			if ( $next ) {
				...
				$desc = yogastudio_strshort($next->post_title, 30);
				?>
