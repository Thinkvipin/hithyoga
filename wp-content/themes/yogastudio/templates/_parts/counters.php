<?php
// Get template args
extract(yogastudio_template_get_args('counters'));
...
if ($show_all_counters || yogastudio_strpos($post_options['counters'], 'views')!==false) {
	?>
	<<?php yogastudio_show_layout($counters_tag); ?> class="post_counters_item post_counters_views icon-eye" title="<?php echo esc_attr( sprintf(__('Views - %s', 'yogastudio'), $post_data['post_views']) ); ?>" href="<?php echo esc_url($post_data['post_link']); ?>"><span class="post_counters_number"><?php echo esc_attr($post_data['post_views']); ?></span><?php if (yogastudio_strpos($post_options['counters'], 'captions')!==false) echo ' '.esc_html__('Views', 'yogastudio'); ?></<?php yogastudio_show_layout($counters_tag); ?>>
	<?php
}
...
if ($rating > 0 && ($show_all_counters || yogastudio_strpos($post_options['counters'], 'rating')!==false)) { 
	?>
	<<?php yogastudio_show_layout($counters_tag); ?> class="post_counters_item post_counters_rating icon-star" title="<?php echo esc_attr( sprintf(__('Rating - %s', 'yogastudio'), $rating) ); ?>" href="<?php echo esc_url($post_data['post_link']); ?>"><span class="post_counters_number"><?php echo esc_attr($rating); ?></span></<?php yogastudio_show_layout($counters_tag); ?>>
	<?php
}
...
if ($show_all_counters || yogastudio_strpos($post_options['counters'], 'likes')!==false) {
	...
	<a class="post_counters_item post_counters_likes icon-heart <?php echo !empty($allow) ? 'enabled' : 'disabled'; ?>" title="<?php echo !empty($allow) ? esc_attr__('Like', 'yogastudio') : esc_attr__('Dislike', 'yogastudio'); ?>" href="#"
		...
		data-title-dislike="<?php esc_attr_e('Dislike', 'yogastudio'); ?>"><span class="post_counters_number"><?php echo esc_attr($post_data['post_likes']); ?></span><?php if (yogastudio_strpos($post_options['counters'], 'captions')!==false) echo ' '.esc_html__('Likes', 'yogastudio'); ?></a>
	<?php
}
