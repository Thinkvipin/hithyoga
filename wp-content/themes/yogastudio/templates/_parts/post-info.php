<?php
// Get template args
extract(yogastudio_template_get_args('post-info'));
$info_parts = array_merge(array(
	...
	if ($info_parts['author'] && $post_data['post_type']=='post') {
		?>
		<span class="post_info_item post_info_posted_by<?php echo !empty($info_parts['snippets']) ? ' vcard' : ''; ?>"<?php echo !empty($info_parts['snippets']) ? ' itemprop="author"' : ''; ?>><a href="<?php echo esc_url($post_data['post_author_url']); ?>" class="post_info_author  icon-adult"><?php yogastudio_show_layout($post_data['post_author']); ?></a></span>
		<?php 
	}
	...
	if (is_single() && !yogastudio_storage_get('blog_streampage') && ($post_data['post_edit_enable'] || $post_data['post_delete_enable'])) {
		?>
