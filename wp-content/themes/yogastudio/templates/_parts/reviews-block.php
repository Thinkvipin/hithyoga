<?php
// Get template args
extract(yogastudio_template_get_args('reviews-block'));
...
if ($avg_author > 0 || $avg_users > 0) {
	...
	if (!$reviews_first_author || !$reviews_second_hide) {
		...
		$field["descr"] = wp_kses_data( sprintf(__("Summary rating from <b>%s</b> user's marks.", 'yogastudio'), $users) 
									. ' ' 
	...
	if ($allow_user_marks) {
        ...
        yogastudio_storage_set_array('js_vars', 'reviews_allow_user_marks', $allow_user_marks);
        yogastudio_storage_set_array('js_vars', 'reviews_max_level', $max_level);
        yogastudio_storage_set_array('js_vars', 'reviews_levels', yogastudio_get_theme_option('reviews_criterias_levels'));
        yogastudio_storage_set_array('js_vars', 'reviews_vote', isset($_COOKIE['yogastudio_votes']) ? $_COOKIE['yogastudio_votes'] : '');
        yogastudio_storage_set_array('js_vars', 'reviews_marks', explode(',', $marks));
        yogastudio_storage_set_array('js_vars', 'reviews_users', max(0, $users));
        yogastudio_storage_set_array('js_vars', 'post_id', $post_data['post_id']);
	}
}
