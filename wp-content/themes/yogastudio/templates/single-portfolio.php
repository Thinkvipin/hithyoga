...
if ( !function_exists( 'yogastudio_template_single_portfolio_output' ) ) {
	function yogastudio_template_single_portfolio_output($post_options, $post_data) {
		...
		yogastudio_template_set_args('prev-next-block', array(
			'post_options' => $post_options,
			'post_data' => $post_data
		));
		get_template_part(yogastudio_get_file_slug('templates/_parts/prev-next-block.php'));
		...
		if ($show_title) {
			?>
			<h1 itemprop="<?php echo (float) $avg_author > 0 || (float) $avg_users > 0 ? 'itemReviewed' : 'headline'; ?>" class="post_title entry-title"><?php yogastudio_show_layout($post_data['post_title']); ?></h1>
			<?php
		}
		...
		if ($post_data['post_protected']) { 
			...
			yogastudio_show_layout(yogastudio_gap_wrapper(yogastudio_reviews_wrapper($post_data['post_content'])));
			wp_link_pages( array(
