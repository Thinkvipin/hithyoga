...
if ( !function_exists( 'yogastudio_template_news_magazine_theme_setup' ) ) {
	add_action( 'yogastudio_action_before_init_theme', 'yogastudio_template_news_magazine_theme_setup', 1 );
	function yogastudio_template_news_magazine_theme_setup() {
		yogastudio_add_template(array(
			'layout' => 'news-magazine',
			'template' => 'news-magazine',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Magazine/', 'yogastudio'),
...
// Template output
if ( !function_exists( 'yogastudio_template_news_magazine_output' ) ) {
	function yogastudio_template_news_magazine_output($post_options, $post_data) {
				...
				<div class="post_header entry-header">
					...
					if ($show_title) {
						if (!isset($post_options['links']) || $post_options['links']) {
							?>
							<<?php echo esc_html($title_tag); ?> class="post_title entry-title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php yogastudio_show_layout($post_data['post_title']); ?></a></<?php echo esc_html($title_tag); ?>>
							<?php
						} else {
							?>
							<<?php echo esc_html($title_tag); ?> class="post_title entry-title"><?php yogastudio_show_layout($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
							<?php
						}
					}
					
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						?><div class="post_meta"><span class="post_meta_author"><?php yogastudio_show_layout($post_data['post_author_link']); ?></span><?php
			...
			if ($number <= $featured) {
				...
				<div class="post_footer entry-footer">
					...
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						...
						yogastudio_template_set_args('counters', array(
							'post_options' => $post_options,
							'post_data' => $post_data
						));
						get_template_part(yogastudio_get_file_slug('templates/_parts/counters.php'));
					}
					?>
					</div>
				</div><!-- .entry-footer -->
