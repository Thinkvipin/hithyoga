/**handles:yogastudio-core-init-script**/
/* global jQuery:false */
...
jQuery(window).on('beforeunload', function() {
	"use strict";
	// Show preloader
//	jQuery('body.preloader>.outer_wrap,body.preloader>.body_wrap,body.preloader>.yogastudio_profiler').css({opacity:0});	//.animate({opacity:0}, 'fast');
...
function yogastudio_init_actions() {
	...
	// Hide preloader
//	jQuery('body.preloader>.outer_wrap,body.preloader>.body_wrap,body.preloader>.yogastudio_profiler').animate({opacity:1}, 'slow');	//.css({opacity:1});
		...
		if (scroll_offset + jQuery(window).height() > YOGASTUDIO_STORAGE['side_panel_height'] + 100) {
			if (jQuery('.sidebar_outer').css('position')!=='fixed') {
				jQuery('.sidebar_outer').css({
					//'top': (scroll_offset + jQuery(window).height() - YOGASTUDIO_STORAGE['side_panel_height'] - 100) + 'px'
					'position': 'fixed',
...
function yogastudio_init_post_formats() {
	...
	} else if (YOGASTUDIO_STORAGE['popup_engine']=='magnific') {
